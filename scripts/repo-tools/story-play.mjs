...
const usage = 'Usage: pnpm story:play <server|list|profiles|starts|show ID|history ID|new [options] --confirm-live|choose ID OPTION --confirm-live|wait ID|retry ID --confirm-live>';
...
function requireLive(action) {
  if (!live) {
    throw new Error(`${action} may dispatch paid Storyteller work; repeat with --confirm-live after reviewing status and choices`);
  }
}
  ...
  return {
    ...
    narrativeChoices:
      narrative?.kind === 'choice.v1'
        ? narrative.options.map(({ id, label, description, risk }) => ({ id, label, description, risk }))
        : [],
...
function pathFor(nodes, id) {
  ...
  while (current) {
    ...
    current = current.parent === null ? undefined : nodes.find((node) => node.id === current.parent);
  }
  return path;
}
...
async function record(kind, value, storyId = 'catalogue') {
  ...
  await appendFile(path, `${JSON.stringify({ at: new Date().toISOString(), kind, value })}\n`, { encoding: 'utf8' });
  return path;
}
...
async function waitForStory(storyId, before, timeoutMs = 120_000, requireChange = true) {
  const deadline = Date.now() + timeoutMs;
  let latest = await readStory(storyId);
  while (Date.now() < deadline) {
    const changed = latest.viewVersion !== before.viewVersion || latest.revision !== before.revision;
    const active =
      latest.resolution && ['pending', 'running'].includes(latest.resolution.state);
    const execution = latest.campaign?.actionExecution;
    const interval = latest.waiting;
    if ((!requireChange || changed) && !active && !execution && !interval) return latest;
    if (latest.resolution && ['failed', 'uncertain', 'blocked'].includes(latest.resolution.state)) return latest;
...
async function createStory() {
  ...
  const pace = {
    slow: { kind: 'rate', ticks: 1, realMs: 60_000 },
    steady: { kind: 'rate', ticks: 1, realMs: 1000 },
    fast: { kind: 'rate', ticks: 10, realMs: 1000 },
  ...
  const [catalogue, profiles] = await Promise.all([
    request('/api/drafts/00000000-0000-4000-8000-000000000000/openings/catalogue'),
  ...
  const draft = await request(`/api/drafts/${draftId}`, {
    ...
    body: JSON.stringify({
      ...
      storytellingDirection: direction || start.draft?.storytellingDirection || '',
    }),
  });
  const openingId = randomUUID();
  await request(`/api/drafts/${draftId}/openings/${openingId}`, {
    method: 'PUT',
    body: JSON.stringify({ expectedRevision: draft.revision, contentId: start.id }),
  });
  let preview;
  ...
  if (!preview || preview.state !== 'succeeded' || !preview.candidate) {
    const path = await record('opening-terminal', { draftId, openingId, preview });
  ...
  const path = await record('created', { draftId, openingId, profile, startId, summary }, storyId);
...
async function retry(storyId) {
  ...
  const admitted = await request(`/api/stories/${storyId}/retries/${randomUUID()}`, {
    method: 'PUT',
    body: '{}',
  });
  const final = await waitForStory(storyId, before);
  const summary = summarize(final);
  const path = await record('retry', { admitted: summarize(admitted), final: summary }, storyId);
  return { ...summary, evidencePath: path };
}
...
async function main() {
  ...
  if (!command || command === 'help' || command === '--help' || command === '-h') {
    ...
    const status = JSON.parse(await readFile(join(evidenceRoot, 'server.json'), 'utf8'));
    const identity = await request('/api/me');
    result = { ...status, reachable: true, identity: { id: identity.id, email: identity.email } };
  } else if (command === 'list') result = await request('/api/stories');
  else if (command === 'profiles') result = await request('/api/storytellers');
  else if (command === 'starts') result = await request('/api/drafts/00000000-0000-4000-8000-000000000000/openings/catalogue');
  ...
  else if (command === 'choose') result = await choose(args.shift(), args.shift());
