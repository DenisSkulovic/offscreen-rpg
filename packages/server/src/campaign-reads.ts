...
import { realMsUntilTick } from './rules/tick-clock';
...
export async function readCampaign(db: Pick<Database['db'], 'select'>, ownerId: string, storyId: string): Promise<CampaignView | null> {
  ...
  const rolls = await db.select().from(gameRoll).where(eq(gameRoll.storyId, storyId)).orderBy(desc(gameRoll.tick), desc(gameRoll.id)).limit(100);
  ...
  if (activity) {
    ...
    activityView = {
      id: activity.id, label: plan.action.label, state: activity.state,
      completed: activity.completed, durationTicks: plan.action.durationTicks,
      revision: activity.revision, resolvedTicks: plan.resolvedThroughTick,
      settingsRevision: plan.settingsRevision,
      dueAt: activity.state === 'running' ? new Date(activity.anchorAt.getTime() + realMsUntilTick(progress, nextBoundaryTick(plan, plan.resolvedThroughTick), pace)).toISOString() : null,
    };
  }
  return campaignViewSchema.parse({
    ...
    tick: state.tick, offer: state.offer, activity: activityView,
    rolls: rolls.map((roll) => ({ id: roll.id, segment: roll.segment, tick: roll.tick, roll: roll.result, effects: roll.effects })),
  });
}
