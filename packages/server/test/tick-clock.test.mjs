...
import { earnedTicks, realMsUntilTick, wholeTicks } from '../src/rules/tick-clock.ts';

const rate = (ticks, realMs) => ({ kind: 'rate', ticks, realMs });
const earn = (progress, pace, now, state = 'running', durationTicks = 100) =>
  earnedTicks({ progress, pace, now, state, durationTicks, anchorAt: new Date(0) });
...
test('reanchoring and changing pace preserve an exact fractional tick', () => {
  ...
  assert.deepEqual(second, { elapsedTicks: 0, remainder: { numerator: '1', denominator: '2' } });
...
test('pause earns nothing, including when changing to instant', () => {
  ...
  assert.deepEqual(earn(progress, { kind: 'instant' }, 90000, 'paused'), progress);
...
test('clamps at completion before converting large integer products to numbers', () => {
  assert.deepEqual(earn(wholeTicks(0), rate(1000000, 1), 8640000000000, 'running', 8), wholeTicks(8));
  assert.deepEqual(earn(wholeTicks(0), { kind: 'instant' }, 0, 'running', 8), wholeTicks(8));
});
...
test('quiet ticks need no work; wakes round up and due boundaries need no wait', () => {
  ...
  assert.equal(realMsUntilTick(wholeTicks(0), Number.MAX_SAFE_INTEGER, rate(1, 86400000)), 86400000);
});
