...
export async function observeLinearMemoryBaseline(args: {
  ...
  const result = await searchCanonicalKnowledge(args.storage, {
...
export async function evaluateLinearMemoryBaseline(args: {
  ...
  for (const query of args.corpus.queries) {
    const observation = await observeLinearMemoryBaseline({
      ...args,
      queryId: query.id,
    });
    reports.push(evaluateMemoryObservation(args.corpus, observation));
  }
