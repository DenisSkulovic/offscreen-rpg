  ...
  const [row] = await tx
    ...
    .where(
      and(
        ...
        eq(campaignSettings.revision, revision),
      ),
    );
...
export function compileCreative(
  ...
  return storytellerProfileSchema.parse({
    ...profile,
    tone: creative.tone,
    dramaticRhythm:
      `${profile.dramaticRhythm}\nEmphasis: ${creative.emphasis}.`.slice(
        0,
        1200,
      ),
    surprisePolicy:
      `${profile.surprisePolicy}\nFrequency: ${creative.surprises}.`.slice(
        0,
        1200,
      ),
  });
...
export async function commandReceipt(
  ...
  const [prior] = await tx
    ...
    .where(
      and(
        eq(campaignCommand.storyId, storyId),
        eq(campaignCommand.operationId, operationId),
      ),
    );
...
export async function ensureCampaign(tx: Transaction, current: StoryRecord) {
  ...
  await initializeCampaign(
    tx,
    current.id,
    storytellerProfileSchema.parse(current.storyteller),
    {
      mechanics: false,
      locked: false,
  ...
  return {
    ...
    async update(args: {
      ...
      await database.db.transaction(async (tx) => {
        ...
        if (
          state.locked ||
          state.settingsRevision !== parsed.data.expectedRevision
        )
          throw new StoryError('conflict');
        ...
        if (parsed.data.presetId) {
          const [preset] = await tx
            .select()
            .from(storytellerPreset)
            .where(
              and(
                eq(storytellerPreset.id, parsed.data.presetId),
                eq(storytellerPreset.ownerId, args.ownerId),
              ),
            );
          if (!preset) throw new StoryError('not_found');
          profile = storytellerProfileSchema.parse(preset.profile);
          if (
            profile.id !== parsed.data.creative.profile.id ||
            profile.revision !== parsed.data.creative.profile.revision
          )
            throw new StoryError('invalid');
        } else if (
          previous.profile.id === parsed.data.creative.profile.id &&
          previous.profile.revision === parsed.data.creative.profile.revision
        ) {
          profile = previous.profile;
        } else {
          try {
            profile = storytellerCatalogue.resolve(
              parsed.data.creative.profile,
            );
          } catch {
            throw new StoryError('invalid');
          }
        ...
        await tx
          .insert(campaignSettings)
          .values({ storyId: current.id, revision, settings, profile });
        await tx
          .update(campaign)
        ...
        await tx
          .update(story)
          .set({ storyteller: compileCreative(profile, settings.creative) })
    ...
    async history(args: { ownerId: string; storyId: string }) {
      return database.db.transaction(async (tx) => {
        ...
        const rows = await tx
          .select()
          .from(campaignSettings)
          .where(eq(campaignSettings.storyId, current.id))
          .orderBy(desc(campaignSettings.revision))
          .limit(20);
        return rows.map((row) => ({
          settings: campaignSettingsSchema.parse(row.settings),
    ...
    async presets(ownerId: string) {
      const rows = await database.db
        .select()
        .from(storytellerPreset)
        .where(eq(storytellerPreset.ownerId, ownerId))
        .limit(50);
      return rows.map((row) => ({
        id: row.id,
        name: row.name,
        creative: creativeSettingsSchema.parse(row.creative),
      }));
    },
    async savePreset(args: { ownerId: string; id: string; body: unknown }) {
      ...
      let profile;
      try {
        profile = storytellerCatalogue.resolve(parsed.data.creative.profile);
      } catch {
        throw new StoryError('invalid');
      }
      await database.db.transaction(async (tx) => {
        const [prior] = await tx
          .select()
          .from(storytellerPreset)
          .where(eq(storytellerPreset.id, id));
        if (prior) {
          if (prior.ownerId !== args.ownerId) throw new StoryError('not_found');
          if (
            prior.name !== parsed.data.name ||
            !isDeepStrictEqual(prior.creative, parsed.data.creative)
          )
            throw new StoryError('conflict');
          return;
        }
        await tx
          .insert(storytellerPreset)
          .values({ id, ownerId: args.ownerId, ...parsed.data, profile });
      });
