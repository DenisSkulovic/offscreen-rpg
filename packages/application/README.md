...
An encounter-state activity retains its identity and progress across immediate responses. A private `resume` plan names its captured action definition and requires exactly one eligible retained row; it does not yet target an instance ID. Starting another process suspends the interrupted/paused row and changes the one advancing pointer. Selecting work while an activity is running rejects. Explicit abandonment, enforced capacity claims and shared-clock correction remain in the [activity foundation plan](../../docs/features/2026-09-18--16-48--activity-processes-and-progress/PLAN.md). In particular, retained progress does not establish correct world chronology when another activity has advanced time in between.
