...
A two-entry accepted activity plan is a separate durable permission, not another source of activities. Both entries must come from the same current authored offer. On completion, optional report evidence freezes first; then the plan compares the exact accepted successor terms with freshly reprojected authorization and rechecks prerequisites/occurrence scope. The same transaction either creates and schedules one successor or exposes a blocked plan. It never selects a substitute. Editing, cancellation, explicit horizons and longer browser-authored itineraries remain U2c work.
