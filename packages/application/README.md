...
2. Pure immediate resolution chooses an automatic or checked outcome once. The application atomically saves the `game_action_receipt`, character effects, explicit story-fact declarations, consumed offer, command receipt and outbox follow-up. Character facts and the capped story-fact collection have separate ownership; quantity minimums and all fact prerequisites are rechecked before rolling. Immediate actions do not create an activity or advance the campaign clock.
