...
A settled but structurally invalid Storyteller answer is not silently edited into play. When the captured cost policy included one repair round and capacity remains, the held story may offer an explicit Retry that asks the same configured route for one complete corrected replacement under the original turn allowance. The committed player action, elapsed fictional time, receipt and story revision are reused rather than replayed. A one-shot or exhausted configuration remains honestly held and cannot acquire repair capacity after failure.
