...
Status: proposed implementation defaults for the next offline POC, reviewed against source at `875f979`. The owner has specified the product direction; this document closes technical seams for the coding handoff, not claims of runtime support. No live inference, deployment or multiplayer behavior is authorized here.
...
The chain alone owns successor admission. A completion hook emits a continuation request; it cannot create a second child. Each start checks the current authored handoff/scope, mechanical eligibility and player permission, atomically consumes its entry and starts once. Quiet automatic transitions occur at their boundary during catch-up. A fresh player click after idling starts at the current settled tick, never retroactively at the prior completion.
