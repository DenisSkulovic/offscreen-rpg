...
New task artifacts have `inputVersion: 12` and `promptVersion: storyteller.v12`; captured v10/v11 work remains readable for repair/replay, and result versions still differ by task. These are implementation versions, not “twelve thinking steps.” The runtime can execute a scripted adapter instead of a provider: this exercises task/validation/publication, but proves neither model understanding nor token-free task admission.
