...
New task artifacts have `inputVersion: 11` and `promptVersion: storyteller.v11`; captured v10 work remains readable for repair/replay, and result versions still differ by task. These are implementation versions, not “eleven thinking steps.” The runtime can execute a scripted adapter instead of a provider: this exercises task/validation/publication, but proves neither model understanding nor token-free task admission.
