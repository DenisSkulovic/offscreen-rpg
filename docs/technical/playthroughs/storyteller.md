...
The task artifact currently has `inputVersion: 3` and `promptVersion: storyteller.v1`; result versions differ by task. These are implementation versions, not “three thinking steps.” The runtime can execute a scripted adapter instead of a provider: this exercises task/validation/publication, but proves neither model understanding nor token-free task admission.
