...
The remaining connected behavior is dispatching this reviewed request through durable multi-round accounting and tracing which evidence the final result actually used. That result may acknowledge the old favor and the current repaired-bridge state, but it may not revive a superseded rumor, manufacture a second favor or treat the historical passage as present state.
