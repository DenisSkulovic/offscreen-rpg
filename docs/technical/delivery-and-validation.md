...
Before any paid playthrough, explain the actual supported loop and proposed run allowance to the user. Do not present a scripted fixture, an opening-only generation or decorative model-written narration as a complete AI game. The intended narrow live POC starts with a user premise, generates a playable opening and contextual choices, commits an accepted action, continues through a saved real-time wait, and generates a coherent response and next choices from the resulting state. The SpongeBob example is acceptance content, not a dedicated game engine.
