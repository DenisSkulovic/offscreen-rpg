...
Keep inspection modest and separate from the story presentation. The Chamber inspector is the observability layer for committed revision, current passage/decision, relevant possessions, timing remainder or deadline, and generation provenance when a passage has it. Keep secrets and hidden generation context out of ordinary player snapshots; option intentions may appear only in the developer inspector. Starting a fresh fixture creates a new story rather than resetting existing history in place. Add failure injection or time-skipping controls only when a particular test requires them; normal UI waits must use the real scheduler. Automated time-skipping tests do not replace a real-service restart check.
