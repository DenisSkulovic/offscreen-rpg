...
The Chamber is a local scenario laboratory for exercising real Offscreen RPG persistence and execution. Developers select authored scenarios by name and purpose; stored source IDs remain versioned `chamber.vN` keys. A read-only inspector shows committed story identity, the current passage, timing/decision fields, items, bounded recent history and generation provenance when present. Inspection is developer-only observability: it must not implement gameplay, mutate committed state, or appear on ordinary production `createApp(...)` unless an explicit local `developerTools` option is set. Fake storyteller injection, time acceleration and fault controls are not connected yet.

The existing chamber exercises short, persistent solo scenarios with authored content and real application behavior. It remains development equipment while the actual creation and continuation lifecycle is connected. This is a development fixture, not a new game mode or a commitment to the final mechanics. It must use the application's story storage, command admission, transition validation, Temporal execution and browser reads. The existing in-memory `/demo` can remain a presentation example; it is not the chamber's source of authoritative state.
...
This establishes a playable narrative branch without making an invented token economy a prerequisite. Extend the connected flow with a short real wait, held response window, permitted timeout default and pause/resume. The letter fixture adds an identity-bearing item transfer committed with its narrative consequence and protected against retries; consumption is not implemented. The timed cafe fixture implements a 20-second wait and background arrival. Journey pause/resume and a separate timed gate reply are connected; one immediate item transfer is connected through the letter fixture; broader inventory remains unimplemented. The gate reply holds fiction for 15 real seconds and defaults to leaving if no answer is accepted. The browser test holds a paused journey past its original deadline with the page closed and worker restarted, resumes its saved remainder, and verifies one arrival. It also checks control authorization, conflicting reuse and retrying an old pause after resume. The immediate conversation alone does not demonstrate offscreen gameplay.

The exact text and example objects are replaceable. Start with seconds-long waits for convenient observation, explicitly separate from fictional duration. For this fixture, quiet waits advance fictional time, response windows hold the fictional situation, and manual journey pause freezes its remaining duration. The separate timed reply does not permit pause; pausable decision windows remain a later policy. Use fixed durations per passage; mid-interval speed changes, continuous travel progress and arbitrary interruption are outside this first experiment. These declared fixture rules do not settle the general pacing or autonomy controls.
...
Keep inspection modest and separate from the story presentation. The Chamber inspector is the observability layer for committed revision, current passage/decision, relevant possessions, timing remainder or deadline, generation provenance, and activity-report hook/source plus generation/publication status. It exposes correlation and state, not raw provider payloads or hidden game plans. Keep secrets and hidden generation context out of ordinary player snapshots; option intentions may appear only in the developer inspector. Starting a fresh fixture creates a new story rather than resetting existing history in place. Add failure injection or time-skipping controls only when a particular test requires them; normal UI waits must use the real scheduler. Automated time-skipping tests do not replace a real-service restart check.

Local access is part of the delivery task. Use configured GitHub sign-in for the normal application, or the `tools/chamber` workspace, which composes the built API, web and worker while seeding a disposable test user/story and authenticated browser session through existing test utilities. Cross-process PostgreSQL, Temporal and browser suites live in `tools/api-integration`, outside deployable package graphs. Never add a public login bypass or weaken owner checks to make the chamber easier to open. The documented launch path must not require LLM credentials.

Success means the short story is playable and inspectable across refresh and worker interruption, with exactly one committed outcome for a repeated choice or competing deadline. It does not demonstrate realistic geography, arbitrary model-generated mechanics, multiplayer resolution, notification delivery or narrative quality. Those need later fixtures and product decisions. See the [remaining implementation slices](../progress.md#next-milestone-connect-the-actual-story-lifecycle-without-inference) for current status.

## First live-story POC: what the run must mean

Live calls are disabled during the current development phase; the available key remains unused. Do not schedule paid checks merely because a fixture is runnable. Before any later paid playthrough, explain the actual supported loop and proposed run allowance to the user. Do not present a scripted fixture, an opening-only generation or decorative model-written narration as a complete AI game. The intended narrow live POC starts with a user premise, generates a playable opening and contextual choices, commits an accepted action, continues through a saved real-time wait, and generates a coherent response and next choices from the resulting state. The SpongeBob example is acceptance content, not a dedicated game engine.

For that run, travel from the pineapple to the restaurant lasts ten real minutes under an explicit test pacing policy. Waiting, polling and reopening consume no model tokens. A prepared arrival may be reused, or a bounded call may resolve arrival; identify which path is implemented before the run. Telling Squidward a story must then produce a context-aware continuation, not return an authored branch while claiming inference. Each generated choice must refer to a supported resolution path; model prose alone cannot create a driving simulation, an inventory effect or a timer.

The first live loop need not have a map, economy, simulated population, free-text actions or multiplayer. Its limitations must appear in the demonstration explanation. A technically valid result can still fail the POC through dull options, contradictory locations, invented possessions, incoherent time or a response that ignores the player's intention. Observe those failures directly; do not silently rewrite bad output and report the repaired story as a model success.

An offline immediate-scene proposal now supplements the opening-prose schema. Current blockers are its connection to candidate review/Start and async generation-to-commit, continuing context beyond one scene, timing/effect integration, and the enforced spending ledger/opt-in adapter. Exercise these with scripted provider responses and failure cases first. Then propose a tiny live schema/usage check, a continuation check and one brief playthrough with an explicit combined cap. Select current cheap models and estimated call costs only when ready; no prices or quality claims are established yet. Record actual charges after every attempt and halt paid work on uncertainty. Availability of the $10 key does not remove these gates.
