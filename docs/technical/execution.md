...
The timed chamber saves a versioned wait plan and server-derived due time with its waiting passage. The same transaction admits a `story.interval.v1` outbox notice. The relay starts `storyIntervalV1` with ID `story-interval/<passage-id>` and rejects duplicate reuse. Only that passage ID enters workflow input. The workflow asks an Activity to advance; if PostgreSQL reports remaining time, it sleeps durably and tries again. Recovery rechecks the original saved due time rather than restarting the full interval.

The Activity locks the story, checks that the waiting passage is still current and compares database time with its due time. An early wake returns the remainder. A due arrival commits through the common continuation transaction using the waiting passage ID as its transition identity. A repeated or no-longer-current completion makes no change. The prepared arrival remains private until publication. No model invocation or per-second database update occurs.

This is a bounded executor for a fixed interval, not the full story command coordinator described below. It closes after its one arrival; a later independently admitted interval has its own identity. Future pause, rescheduling and shared decisions require durable command/control fencing and coordination before they are exposed. The fixed fixture does not support those controls, an adjustable game clock or custom calendar scheduling. Its declared fictional duration and fixed real duration define the initial mapping only. A future story coordinator may compose these bounded operations; they must not become an independent competing authority for a paused or rescheduled story.

While a saved interval is current, the browser reads the snapshot every two seconds, with no overlapping requests and cancellation on unmount. Only the server publishes arrival. This is a small polling transport until SSE is justified; it stops when the interval leaves the current snapshot. The page shows the persisted due time as an estimate, including the possibility of delayed worker processing.
