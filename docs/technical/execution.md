...
A generated-option notice starts `scriptedContinuationV1` with Workflow ID `scripted-continuation/<operation-id>`. Only the operation UUID enters history. The Activity writes the same deterministic playable proposal, then commits the next passage only if the story still matches that resolution's base passage and revision. A late success remains generation evidence and does not rewrite a story that has moved on. Repeated Activities cannot insert a second passage for the same operation. This fixture has no provider call; do not copy its retry policy to paid adapters.
