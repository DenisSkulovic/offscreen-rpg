...
A generated-option notice starts `scriptedContinuationV1` with Workflow ID `scripted-continuation/<operation-id>`. Only the operation UUID enters history. The Activity writes a deterministic continuation result, then commits only if the story still matches that resolution's base passage and revision. Immediate output publishes a playable scene. Timed output publishes the in-progress passage and an ordinary wait plan; arrival stays prepared until the existing interval executor commits it. A late success remains generation evidence and does not rewrite a story that has moved on. Repeated Activities cannot insert a second passage for the same operation. This fixture has no provider call; do not copy its retry policy to paid adapters. The scripted generated wait is two real seconds and is fixture policy, not player pace.

The application worker and relay share a process for this slice, but their code and responsibilities are separate. Shutdown stops relay polling and drains worker execution before closing connections. Native and client connection closes run independently so one failure cannot skip the other; collected failures still surface after both attempts. The local chamber launcher uses the same independent-release rule for browser, worker, web process, API and database. The current connection settings target local development; hosted authentication, deployment/versioning policy, production monitoring and recovery administration remain future work. The preview flow does not implement gameplay; the fixed interval flow below adds the first story wait, without provider calls.

## Implemented fixed interval

The timed chamber saves a versioned wait plan and server-derived due time with its waiting passage. Intervals use a `story.interval.v1` outbox notice. The relay starts `storyIntervalV1` with ID `controlled-interval/<passage-id>` and rejects duplicate reuse. Only the passage ID enters workflow input.
