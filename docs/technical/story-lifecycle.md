...
`opening.scripted.v1` is a separate, fixed fixture kind. Its local completion atomically saves the known output for a pending request, so a retry can recover interruption after admission. There is no external side effect, running claim or model call in that path; preserve its versioned fixture during recovery. This narrow synchronous fixture runner must never acquire a provider call. Outbox dispatch and Temporal execution remain unimplemented; real background work must use the generic claim/uncertainty rules below. The UI visibly describes the sample as not adapted to the premise.
