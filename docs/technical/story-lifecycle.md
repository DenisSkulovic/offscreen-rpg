...
The chamber is a separate authored fixture, not a generated preview converted into live play. Fresh browser starts use `PUT /api/stories/:id/chamber` with `{ scenario: "chamber.v2" }`. It requires a verified owner and configured write origin. The server selects the content, and initialization returns a saved snapshot. The earlier `chamber.v1` fixture remains readable and retryable with its original source/content; it does not accept responses. New fixtures use new IDs, never reset existing history.
...
These particular choices resolve synchronously with short database work, no external calls, timers or pending generation. They return 200 after commit and do not create an idle Temporal workflow. This does not replace the durable inbox/202 contract for asynchronous gameplay work. When timing or generation is connected, admission, control fences and asynchronous status must be integrated before offering those operations. The fixture resolver cannot process arbitrary generated stories; no client or model can supply outcome prose through this endpoint.

The first playable branch approaches a gate, offers a reply and ends with a joke or departure. It tests persistent narrative consequences, not item transfers, a lifecycle-wide terminal flag or autonomous progression. A null interaction is not a universal definition of a finished story. The page shows identifiers/revision and an on-demand chronology reader. The generated/shared start contract below remains unimplemented.
