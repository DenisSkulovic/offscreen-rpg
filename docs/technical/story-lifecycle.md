...
A current successful candidate is sufficient for Start: the owner submits the candidate generation ID and expected draft revision with a client-stable story UUID. Start does not perform inference, regenerate the scene or copy hidden intentions into the live passage. Spending limits and account-wide concurrency admission must still precede public or paid generation. A worker must check whether the input is still relevant before spending; storing stale results does not make them current.
...
The response is the ordinary story snapshot. `canRespond` is false: generated options are visible but must not enter the chamber fixture resolver. Reopening uses `GET /api/stories/:id`. The live player route is `/play/:id`.

A retry with the same story ID and same candidate returns the existing story. Reusing that story ID with a different candidate or incompatible initialization returns 409. A stale, superseded, failed or malformed candidate cannot start. A candidate owned by someone else returns 404. Chamber initialization remains a separate `PUT /api/stories/:id/chamber` path and does not require generation provenance.

Asynchronous start receipts, live generation, group readiness and funding checks remain unimplemented. Selecting a generated option and resolving a continuation is the next slice.
