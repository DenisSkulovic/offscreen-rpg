...
The first playable branch approaches a gate, offers a reply and ends with a joke or departure. It tests persistent narrative consequences, not item transfers or a lifecycle-wide terminal flag. The timed visit adds automatic arrival and saved journey pause/resume; pace changes and response defaults remain absent. A null interaction is not a universal definition of a finished story. The page shows identifiers/revision and an on-demand chronology reader. The generated/shared start contract below remains unimplemented.
