...
Status: B1 and the first B2 quota slice implement pure policy resolution, fail-closed task admission and durable per-attempt window allocations. A provider task captures its complete resolved authority, route match and narrowed envelope; started stories retain that authority for later tasks. Reservation atomically covers every captured fixed/rolling request, token, money and background-job window; unsent work releases it, known settlement records actual consumption, and uncertainty retains the conservative allocation. Commercial account selection, operation-wide multi-attempt aggregation, player holds/recovery and billing remain prepared work. The [bounded-cost feature](../features/2026-09-19--19-08--bounded-storyteller-cost/PLAN.md) owns delivery. [Context and cost](context-and-cost.md) owns request construction and operation accounting. Commercial prices, tier names and payment processing are not chosen here.
...
Do not encode product geometry such as `free | standard | premium` into gameplay or context assembly. Named commercial offerings map to versioned grants over independent dimensions, and advanced users may apply stricter personal limits or explicitly funded larger envelopes. The same resolver must support a minimal sponsored check and an unusually expensive high-context session without special-case branches. A million-token-capable route does not justify transmitting a million tokens; admission permits an envelope and the task recipe spends only evidence with measured expected value.
...
The implemented one-shot runtime persists one allocation per captured window and attempt under the existing global accounting lock. Fixed windows store their anchored half-open period; rolling windows retain their dispatch attribution time. Settled allocations count only in their applicable period, while reserved/dispatched/uncertain allocations continue to constrain admission across a boundary. Story-scoped windows currently require an existing story identity, so pre-story opening work must use applicable platform/account windows rather than silently minting a temporary story scope.
