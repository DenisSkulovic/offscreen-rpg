...
Status: the execution gate, finite-action boundary, fail-closed bounded-overlap path and fictional-second coordinate are implemented with deterministic offline evidence. Runtime clock projection requires an explicit accepted activity or finite-action identity; no-hold state alone cannot advance time, and new work starts from the settled frontier. An eligible bounded action draws once under admission, stores a private pending result with source/projected-state digests and prepares one explicitly pending Storyteller task during its wait. Settlement revalidates and promotes that exact result and generation to the authoritative receipt before publication; effects, facts, history, holds and choices remain current-state operations only at or after the target. Any due admitted world obligation forces the sequential path. Chamber/API rehearsals cover early preparation during pause, late held preparation across worker restart, explicit same-generation failure recovery and due-boundary interruption. [Time and autonomy](../time-and-autonomy.md) owns the product intent; [fictional-time arithmetic](ticks-and-tags.md) owns units. This document owns execution permission and its relationship to narration.

## Units and responsibility — current implementation

The scheduler advances in fixed real-second steps while model-proposed durations, canonical time, cadence and deadlines use fictional seconds. Pace states how many fictional seconds are earned per eligible real second. At 360 fictional seconds per real second, a 1,800-fictional-second action consumes thirty fictional minutes and five active real seconds. Doubling pace changes only the wall wait; the action still advances 1,800 fictional seconds and has the same rule boundaries. Thirty fictional minutes is appropriate for a careful search, not automatically a quick glance. Numerical examples are not universal defaults.

The scheduler heartbeat is not the unit of mechanical truth. Otherwise changing speed could make one work attempt represent different amounts of fictional labor. Keep check cadence, duration, quantities and world deadlines in the stable fictional-second coordinate. No mandatory database write, RNG draw or provider request per real second. Use meaningful boundary wakes and bounded ordered settlement. Defined calendar units have exact positive mappings; custom dates require a captured definition and epoch. Never recover executable units from prose.
...
Public mechanical offers now carry explicit timing. Automatic/check actions expose their validated positive fictional duration; process/resume choices say `process`. Selecting a finite action creates a durable execution rather than a receipt: the execution owns clock eligibility until its target game second, after which mechanics settle and narration begins. This removes the earlier ambiguous missing-value and zero-time contracts without representing dialogue as contribution work.
...
The private plan schema requires `fictionalDurationSeconds` for every automatic/check action; missing, zero, negative and over-bound values fail admission. Activity process rules remain a distinct progress dimension. The admitted duration is projected publicly and persisted with immutable start and target game seconds before any outcome is rolled or applied. Zero time is appropriate for application controls and specifically supported instantaneous rules; ordinary dialogue, looking around, eating and combat attempts cannot become free merely because their result is atomic. The Storyteller proposes duration within supported content/rule bounds. The application rejects missing or unsupported timing rather than silently inventing a default five seconds for every option. UI grouping/navigation is not an in-world action.
...
| --- | --- | --- |
| Tavern conversation | Pace 60 fictional seconds/real second. Three exchanges take 60, 120 and 30 fictional seconds | World advances by 210 fictional seconds; reading and generation latency add zero; each action has one receipt |
| Careful search, early prose | 1,800 fictional seconds at 360 fictional seconds/real second; preparation ready at real second 2 | At second 2 no result/effect is current; execution ends and next choices publish at second 5 |
...
| Interrupted repair + conversation | Repair suspends at game second 10 with 3 points; dialogue spends 2 + 3 fictional seconds | World reaches game second 15, repair still has 3 points; explicit resume continues work there |
