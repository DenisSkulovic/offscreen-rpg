...
The private plan schema now requires `durationTicks` for every automatic/check action; missing, zero, negative and over-bound values fail admission. Activity process rules remain a distinct progress dimension. The duration is not projected publicly yet because current settlement remains immediate; public claims must follow implemented execution, not lead it. Zero time is appropriate for application controls and specifically supported instantaneous rules; ordinary dialogue, looking around, eating and combat attempts cannot become free merely because their result is atomic. The Storyteller proposes duration within supported content/rule bounds. The application rejects missing or unsupported timing rather than silently inventing a default five seconds for every option. UI grouping/navigation is not an in-world action.
