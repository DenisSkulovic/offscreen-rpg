...
The owner now requires a fixed real-second tick and model-proposed fictional durations. The [time redesign boundary](ticks-and-tags.md#target-one-real-second-tick) replaces the unit definitions below; execution permission and narration/settlement authority remain applicable. No runtime conversion has been implemented yet.
...
Public mechanical offers now carry explicit timing. Automatic/check actions expose their validated positive tick duration; process/resume choices say `process`. Selecting a finite action creates a durable execution rather than a receipt: the execution owns clock eligibility until its target, after which mechanics settle and narration begins. This removes the earlier ambiguous missing-value and zero-time contracts without representing dialogue as contribution work.
