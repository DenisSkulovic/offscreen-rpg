...
The captured input has `inputVersion: 1` and `promptVersion: opening.v1`. Changing generation instructions requires changing the prompt version. The opening application module persists the exact request artifact so recovery does not rebuild it using newer instructions. `openingArtifactSchema` validates that stored version; retain compatible readers as versions evolve. Provider calls, repair loops and generated preview UI are not implemented.
