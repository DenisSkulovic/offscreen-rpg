...
This candidate can be started as a live first passage. Start maps the accepted generation onto an owned story without inference, timers, possessions or autonomous choices. Authorization, currentness and stale-result checks belong to that application operation. Generated option resolution uses the same playable proposal contract through a scripted continuation Activity, not a live provider.
