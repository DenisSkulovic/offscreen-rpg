...
The captured input has `inputVersion: 1` and `promptVersion: opening.v1`. Changing generation instructions requires changing the prompt version. The opening application module persists the exact request artifact so recovery does not rebuild it using newer instructions. `openingArtifactSchema` validates that stored version; retain compatible readers as versions evolve. The browser can display a persisted scripted sample through the separate fixture path described in [lifecycle](story-lifecycle.md). Provider calls and repair loops are not implemented.
