...
This component has no public arbitrary-content write endpoint. The chamber response endpoint calls it only with a server-selected authored consequence. It validates an answer against the saved offer but does not interpret its meaning, validate possessions, enforce story-wide pause, or invoke a storyteller. New responses record `player` or `default` provenance; existing unlabelled rows remain null. Participant identity and group submission attribution still need a reviewed membership contract. Replacing an unanswered offer is not permitted through this ordinary append path; future interruption needs explicit cancellation/invalidation semantics. Before connecting asynchronous gameplay or inventory, the same transaction must enforce the applicable command/control fences and commit supported effects. Do not call it directly with model or browser output and mistake shape validation for a legal consequence. The current bounded proposal contains no external callbacks or arbitrary state patches.
