...
This component has no public write endpoint and is exercised through server integration tests. It does not interpret player input, validate possessions, enforce pause/deadlines, or invoke a storyteller. Before connecting gameplay, the same transaction must enforce the applicable command/control fences and commit supported effects. Do not call it directly with model or browser output and mistake shape validation for a legal consequence. The current bounded proposal contains no external callbacks or arbitrary state patches.
