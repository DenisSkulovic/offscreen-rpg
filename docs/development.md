...
The foundation contains a TypeScript workspace, Next.js web application, NestJS API, PostgreSQL persistence and Better Auth identity/session handling. GitHub is the first OAuth provider. Signed-in users can create, save and reopen private story drafts, then request and reopen a fixed scripted opening preview. A Temporal worker completes the scripted previews in the background. The signed-in `/chamber` page saves and reopens a scripted story opening and offers a paginated history reader; its choices, possessions and timers are not connected. Shared setup and persistent playable progression are not implemented yet. See [implementation overview](progress.md) for coverage and current priorities. Paid services remain disconnected during broader application development.
