...
The foundation contains a TypeScript workspace, Next.js web application, NestJS API, PostgreSQL persistence and Better Auth identity/session handling. GitHub is the first OAuth provider. Signed-in users can create, save and reopen private story drafts, then request and reopen a fixed scripted opening preview. A Temporal worker completes the scripted previews in the background. The signed-in `/chamber` page offers a short scripted branching conversation with persistent choices, conclusions and paginated history. The default timed cafe fixture saves a 20-second wait and the worker publishes arrival even with the page closed. Pause, pace controls, possessions and shared setup remain unimplemented. Keep the worker running for the visit; without it, the waiting scene remains pending. See [implementation overview](progress.md) for coverage and current priorities. Paid services remain disconnected during broader application development.
