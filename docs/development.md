...
Run `pnpm storyteller:packet-audit` for the faster pure structural comparison of narrative opening, mechanical opening, continuation, consequence and report requests. The default audit also includes human and nonhuman fifteen-turn continuity cases whose independently declared requirements expose the current six-optional-passage loss. It builds only the Storyteller package, uses deterministic authored fixtures, performs no HTTP/database/provider work and writes a fresh `manifest.json` plus `comparison.txt` beneath the reported temporary directory. Select cases with `-- --cases=scene-continuation,activity-consequence` or a profile with `-- --profile=absurd-action-comedy`. Byte counts and common prefixes are exact; token estimates, provider cache hits and savings remain unknown. This audit complements rather than replaces the authenticated held-packet probe.
