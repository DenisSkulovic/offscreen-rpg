...
The owner clarified the target on 2026-09-22: **one tick is always one real second**. Speed determines how much fictional time an eligible tick advances; tick length is not a player setting. The Storyteller estimates an action in fictional time, and the engine derives its real wait and tick estimate from the current speed. A world without calendars still has elapsed in-game seconds; human days, months and named dates remain optional content projections.

This target is not implemented. Current code uses fixed simulation ticks plus a configurable real-time rate, and mechanical model output supplies `durationTicks`. That coordinate must not simply be reinterpreted as real ticks: existing durations, checks and deadlines depend on it. [Tick arithmetic](technical/ticks-and-tags.md#target-one-real-second-tick) defines the redesign boundary. References to simulation ticks in the existing execution/calendar descriptions denote the current runtime coordinate until that transfer is implemented.
