...
An interactive event suspends incompatible queued transitions; the next activity cannot start behind an unresolved fight. After resolution, revalidate remaining intentions and resume only when captured policy permits. A report-only narrated milestone may accompany continued work; it records the earlier boundary if delivered late. Queue completion follows its accepted reporting/continuation policy, ending idle, entering an admitted next routine or presenting a configured interaction. It cannot silently invent a new agenda. Maximum unattended horizon, reporting, response and spending permissions are separate. Detailed chain admission and configurable follow-ups belong to the [autonomy plan](features/2026-09-19--00-26--bounded-autonomy-and-reentry/PLAN.md).
