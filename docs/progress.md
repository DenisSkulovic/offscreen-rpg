...
**Next action:** Complete B2c phase 2 provider-free: explicit Retry must build and persist one complete repair packet, allocate a second attempt beneath the same generation operation, recheck authority and settle without replaying mechanics. Phase 1 now implements immutable `repairable-turn.v1` resources and private retention of an invalid candidate plus compact validator findings; `single-turn.v1` remains unchanged. Fresh Character-Driven Drama story `1b42fa1b-7732-4311-88d1-033b182139dc` remains honestly held because its older one-shot task cannot gain repair authority retroactively. The run has two successful Storyteller turns, so it is evidence—not yet the five-turn diagnostic or POC. No further paid call is justified until the repair path and exact packet are inspected. Gameplay quality remains judged against the captured story and Storyteller configuration. UI polish is lower priority.
...
| Storyteller tasks | Opening, continuation, consequence and report-only contracts; bounded context, multi-round retrieval, execution, publication and recovery | One live retrieved-evidence composition and two-turn ordinary gameplay path exist; reliability and later-recall publication remain unproved |
...
| Provider/accounting | Explicit opt-in adapter, durable reservations, usage windows, dispatch authority recheck, safe blocker projection; captured ordinary one-shot/repairable recipes and private invalid-candidate diagnostics | Explicit ordinary repair dispatch/publication, commercial authority selection and the broader failure/hold QA matrix remain |
