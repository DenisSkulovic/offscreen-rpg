...
The current choice resolver branches on fixture source, narrative revision and option ID. It cannot interpret a newly generated offer. The AI package prepares opening prose only; it does not yet define playable proposals, assemble continuing-story context or resolve player intentions. These are application gaps, not problems that buying a model call would solve.
...
The immediate implementation task is the playable creation/resolution contract, beginning with a concrete fake-provider flow. It needs review before migrations: what the storyteller may propose, what remains server authority, how offered intentions are resolved, and what input is frozen while work is pending. No live-provider dependency is needed to resolve these questions.
