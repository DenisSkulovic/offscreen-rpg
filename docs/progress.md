...
| Private drafts | Working: create, save, list and reopen; PostgreSQL ownership/revision checks and two-tab browser test. | Three text fields; a current playable candidate can start a live first passage. Shared setup absent. [Creation](story-creation.md). |
| Opening preparation and request/result storage | Connected scripted flow: request a deterministic playable candidate from a saved draft, save/reopen it and identify stale results. Protected endpoints omit internal prompts and option intentions. Generic processing lifecycle remains independently tested. | The sample is explicitly not adapted to the premise. Background completion through Temporal; no provider adapter. [Lifecycle](technical/story-lifecycle.md), [AI runtime](technical/storyteller-runtime.md). |
...
| Story start, progression and chronology | Scripted initialization connected: owned story and first passage saved atomically, duplicate starts reuse identities, `/chamber?id=...` reopens the snapshot. A current playable candidate can also start an owned live passage 1 at `/play/:id`, with generation provenance retained. Bounded history retrieval and a browser reader are connected. | Authenticated scripted chamber choices connect to saved branches and conclusions. A generated live choice admits pending resolution and a scripted fake storyteller can commit the next generated passage, including after reload and worker restart. A 20-second timed visit is connected; journey pause/resume is connected, while a separate timed gate reply has an automatic default. The letter fixture exposes a saved item holder. Standard sign-in needs OAuth setup; `pnpm chamber` offers a loopback-only developer session. [Lifecycle](technical/story-lifecycle.md), [data](technical/data.md). |
...
| Scene UI, updates and returning | Browser prototype: `/demo` shows scripted scenes, choices, manual waits, pause/resume, endings and chronology without sign-in. | The `/demo` remains in-memory; `/chamber` is a local laboratory with a descriptive scenario catalog, production snapshots for play, and a read-only inspector of committed state. The timed visit polls saved progress, including while paused, and orders snapshots by a separate view version; no SSE or general reconnect protocol. Presentation is separate from the demo script. [Experience](player-experience.md). |
...
| AI context, usage limits and quality | Offline playable opening requests persist as generation input; validated proposals including intentions are stored. Scripted continuation uses the same playable proposal contract without inference. Package checks run without inference. | Context assembly, simulated usage accounting and narrative evaluation fixtures still absent. Real model routing, cache effectiveness and billed costs remain unverified. [Context and cost](technical/context-and-cost.md). |
...
Apply [the engineering standard](engineering/code-quality.md) before further implementation. Live gameplay still uses a scripted fake storyteller rather than a model.

## Current assessment

The chamber is useful integration equipment, not an investor-facing POC or the next product milestone. It proves real persistence and execution with authored inputs. Chamber scenarios now have descriptive catalog metadata, and an authenticated local launcher can inspect committed story/timing/item/history state without changing it. Fake storyteller, time-warp and fault-injection controls are not present. A saved premise can produce a persisted playable opening candidate, including retained option intentions, while the public preview shows only scene prose and choice labels. Explicit Start converts that accepted candidate into a persistent live first passage and freezes the draft premise onto the story. Generated options resolve through durable scripted continuation rather than the chamber fixture responder. Interactive Chamber control over fake outcomes remains a later laboratory phase.

Authored fixtures still branch on fixture source, narrative revision and option ID. Generated live stories recover hidden intentions from passage generation provenance and admit a generic continuation generation. Continuing memory retrieval, waits/effects from proposals and live-provider integration remain absent. Those remaining gaps are application and policy work, not problems that buying a model call would solve.
...
The local database suite passed 9 checks and the application integration suite passed 32 checks. Integration coverage includes identity, drafts, previews, chamber initialization, saved choices/history, deadline contention, browser closure and worker restart during a paused journey. Item tests cover transfer/reload, retries, rollback, story isolation and rejection of client-supplied effects. The separate `pnpm chamber --smoke` check passed local session provisioning, catalog/inspector visibility, letter delivery/reload and anonymous rejection.

These checks establish behavior within supported fixtures and the connected generated-resolution path. They do not establish narrative quality, a coherent generated world, long-lived memory, shared play, large-scale capacity, spend enforcement or a polished player experience. The local launcher is available without OAuth credentials; the ordinary hosted sign-in path still needs provider setup. Model spend remains disabled; no provider adapter is active. The connected generated loop is:
...
| --- | --- |
| 1. Define and connect playable creation | A saved draft can request, persist and review a playable opening candidate, including server-only intentions. Explicit Start joins that accepted candidate into one owned story with generation provenance. Stale candidates and duplicate starts are rejected. Selecting a generated option is connected through asynchronous scripted resolution. |
| 2. Resolve an intention asynchronously | Admit a selected published option, expose pending resolution, ask a fake storyteller for a proposal, validate and commit it once. Delayed, malformed, stale and failed results and reopen while pending are the remaining hardening cases around this connected path. Newly supplied option IDs work without adding fixture-specific branches to the runtime. Keep effects restricted to implemented operations. |
...
The immediate next task is bounded continuation context, waits/effects remaining out of generated proposals, and Chamber control over fake outcomes. No live-provider dependency is needed.
