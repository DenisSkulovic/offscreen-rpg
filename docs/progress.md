...
**Current focus:** implement the [earned-time playable loop](features/2026-09-19--00-26--earned-time-playable-loop/FEATURE.md). Its runtime separates elapsed clock progress from contribution, resolves and records a D&D check at each productive boundary, and derives a conditional completion estimate from expected contribution. The beacon benchmark can enter that runtime, encounter an event, resolve an immediate response and resume the same saved commitment; end-to-end interruption publication, completion/recap and wall-clock evidence remain next. The [Storyteller agency and taste](features/2026-09-19--00-26--storyteller-agency-and-taste/FEATURE.md) structural gates are implemented and verified, with only owner taste judgment outstanding. [Bounded autonomy](features/2026-09-19--00-26--bounded-autonomy-and-reentry/FEATURE.md), [inhabitable return](features/2026-09-19--00-26--inhabitable-play-and-return/FEATURE.md), and [premise-driven setup](features/2026-09-19--00-26--premise-to-playable-campaign/FEATURE.md) remain approved roadmap work, not implemented behavior.
...
The same opaque public attempt can reference a private contribution-process or resume plan. Starting records a zero-progress commitment; an event can suspend it in `encounter`; immediate responses leave that activity intact; a fenced resume plan reactivates the same activity ID, clock cursor and earned contribution. Starting a different process supersedes interrupted work. Explicit abandonment is still missing, and the current campaign record permits only one active commitment even though content names a capacity.
