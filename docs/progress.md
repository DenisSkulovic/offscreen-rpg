...
The repository structure has now been reviewed against its actual dependency graph. The first three phases of the [monorepo architecture rework](features/2026-09-18--17-58--monorepo-architecture-rework/FEATURE.md) establish `@offscreen/game` as the framework-free rules owner, rename the Storyteller package around its real product role and organize `@offscreen/application` behind capability facades. The remaining phases separate Chamber orchestration from the API build graph, thin the Next.js route tree and align residual tooling/documentation. This remains a bounded reorganization toward the playable POC.
