...
**Next action:** Review and enable the proven two-round Story-mode policy for newly captured tasks, then spend at most one deliberately selected live turn on gameplay evidence. Explicit invalid-output repair now works provider-free: it retains the private candidate/findings, exposes Retry only while the captured second round remains available, allocates beneath the original operation and publishes once without replaying mechanics. `single-turn.v1` remains unchanged. Character-Driven Drama story `1b42fa1b-7732-4311-88d1-033b182139dc` remains honestly held because its older one-shot task cannot gain repair authority retroactively. Gameplay quality remains judged against the captured story and Storyteller configuration. UI polish is lower priority.
