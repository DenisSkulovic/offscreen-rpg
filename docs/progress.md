...
**Next action:** Preserve the now-observed real agent loop. In a corrected missing-context Greywake packet, Sol emitted a valid `needs_context` action, the application executed one canonical lexical read, and the native final grounded its scene, evidence citations, document revision and four distinct choices in the returned records. The final was cut off only because the 2,048-token operation envelope was split evenly: the tiny decision used 68 of 1,024 tokens while the scene hit its 1,024-token limit. The runtime now assigns 256 to decision rounds and 1,792 to the final without increasing total tokens or spend authority; provider-free packets prove the initial answer is absent and the final evidence contains it. Application 17/17 tests and Chamber build pass. Stop provider work here, commit the checkpoint, and only later consider one bounded confirmation run. This establishes genuine model-directed retrieval and grounded composition for one case—not reliability, universal gameplay quality or a need for an unbounded file-browsing agent. Gameplay quality remains judged against the captured story and Storyteller configuration. UI polish is lower priority.
