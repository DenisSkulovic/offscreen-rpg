...
| Story start, progression and chronology | Interaction contract component only: versioned single-choice offers and answers, stable IDs and offered-option validation. | No live story storage, transitions or connected interaction UI. Define the remaining passage/effect contract; commit an opening once and retain subsequent passages. [Gameplay](gameplay.md), [lifecycle](technical/story-lifecycle.md). |
...
These are implementation slices, not time estimates or completion percentages. Slice 1 has begun with the interaction contract; live story persistence and the other slices remain unimplemented. They reuse the existing foundation rather than requiring a new infrastructure phase.

| Slice | Work and completion criterion |
| --- | --- |
| 1. Start and reopen a real story | Single-choice interaction validation exists and is tested independently. Review the remaining passage/effect contract, then add owned live-story state and ordered chronology. Start the selected scripted fixture once, display its committed opening and reopen it after refresh. Resolve local access using existing authentication/test utilities. Current opening prose is not yet a startable gameplay payload. |
