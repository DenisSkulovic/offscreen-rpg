...
The chamber is useful integration equipment, not an investor-facing POC or the next product milestone. It proves real persistence and execution with authored inputs. Chamber scenarios now have descriptive catalog metadata, and an authenticated local launcher can inspect committed story/timing/item/history state without changing it. Fake storyteller, time-warp and fault-injection controls are not present. A saved premise can produce a persisted playable opening candidate, including retained option intentions, while the public preview shows only scene prose and choice labels. Explicit Start now converts that accepted candidate into a persistent live first passage. Generated options remain visible but unresolved; the chamber continues to resolve only authored fixtures. More fixtures or a longer timer would not connect continuation generation.

The current choice resolver branches on fixture source, narrative revision and option ID. It cannot interpret a newly generated offer. The AI package's offline playable-proposal component is connected through draft candidate persistence and Start into live passage 1, not through option resolution. Continuing memory retrieval and generation-to-commit integration remain absent. These are application gaps, not problems that buying a model call would solve.
...
The immediate next task is durable pending resolution of a selected generated option, including stale-result checks. No live-provider dependency is needed.
