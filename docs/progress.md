...
**Current focus:** implementation of the shortest playable DM loop and review of its long-story context boundary. Direct receipts, bounded emergent state and one complete offline DM-turn contract are implemented. The next runtime slice connects the same planning contract to opening review and exercises consecutive browser rounds. The [memory proposal](features/2026-09-18--20-09--storyteller-memory-and-recall/FEATURE.md) identifies concrete limitations in the current note window and defines a later phased extension for review.
...
| Storyteller tasks | Opening, continuation and consequence tasks; consequence returns prose plus zero-to-four freshly validated private plans; bounded context, immutable requests, execution, publication and recovery | Mechanical opening still starts from authored plans; no pre-narrative retrieval rounds yet |
...
Immediate selection resolves directly without an activity row or tick advance. Its receipt, optional roll, effects, story-fact declarations, consumed offer and follow-up commit before a separate worker operation prepares a DM turn. The offline task now returns consequence prose and fresh private plans; application publication revalidates those plans and stores their public projection atomically with the new passage. The next boundary is using this planning contract for opening review and proving consecutive browser rounds.
