...
1. **Connect durable execution.** Draft-to-scripted-preview now works through HTTP and persisted results. Next establish the worker/outbox boundary against this flow, with dispatch and restart tests. The synchronous fixture is not a substitute for background execution. Stop short of expanding provider routing, billing or reconciliation machinery.
