...
**Next action:** Preserve the matched opening and `greywake-hidden-key.v1` evidence. The strict-schema defect is fixed provider-free: optional domain fields are nullable only on the OpenAI wire, synthetic nulls are removed before unchanged validation, and the prior grounded Sol candidate now validates locally. Memory rounds are also structurally separated: a context-capable round can only request evidence or declare readiness, while every publishable answer gets a reserved native-schema final round. The database-backed flow and a fresh zero-attempt packet projection pass; the projected one-read final body is 27,434 bytes under hash `4bdf00d4088445b5266f17d4219702da0a60428abdbd3d40532a511576f57643`. The latest paid Sol probe spent USD 0.006320 but emitted an obsolete final wrapper on the decision round, so it performed zero reads and did not exercise the fixed live final path. Stop provider work here. Inspect and commit this checkpoint; only later consider one deliberately bounded `needs_context -> read -> native final` run. Gameplay quality remains judged against the captured story and Storyteller configuration—the shared engine owns authority and consistency, not pace, flavor or dramatic taste. UI polish is lower priority.
