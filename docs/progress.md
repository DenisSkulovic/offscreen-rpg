...
**Current focus:** next implement [multiple suspended commitments](features/2026-09-19--02-24--multiple-suspended-commitments/FEATURE.md) as an evolution of the [earned-time playable loop](features/2026-09-19--00-26--earned-time-playable-loop/FEATURE.md). The beacon runtime already separates elapsed clock progress from contribution, handles an event and can resume the same interrupted activity. The next correction lets a character suspend activity A, perform activity B, and later resume A after prerequisites and capacity are revalidated. The [Storyteller agency and taste](features/2026-09-19--00-26--storyteller-agency-and-taste/FEATURE.md) structural gates are implemented and verified, with only owner taste judgment outstanding. [Bounded autonomy](features/2026-09-19--00-26--bounded-autonomy-and-reentry/FEATURE.md), [inhabitable return](features/2026-09-19--00-26--inhabitable-play-and-return/FEATURE.md), and [premise-driven setup](features/2026-09-19--00-26--premise-to-playable-campaign/FEATURE.md) remain approved roadmap work, not implemented behavior.
...
The same opaque public attempt can reference a private contribution-process or resume plan. Starting records a zero-progress commitment; an event can suspend it in `encounter`; immediate responses leave that activity intact; a fenced resume plan reactivates the same activity ID, clock cursor and earned contribution. The next approved correction replaces automatic abandonment when starting different work: campaigns will retain multiple unfinished commitments, schedule only the running one, reacquire capacity on resume and reserve abandonment for an explicit decision or irreversible rule consequence.
...
For the current earned-time work, 29 Storyteller tests and 19 game tests pass. Storyteller, game, application, worker, web and API-integration builds pass. The focused PostgreSQL/Temporal Storyteller integration passes 10/10, including atomic beacon admission and resuming the same activity with nonzero saved progress. Worker startup imports its activity directory entrypoint explicitly so a stale legacy build artifact cannot shadow consequence registration. Earlier `pnpm chamber:review` evidence covers the immediate mechanical flow, not this beacon process. This proves wiring and persistence, not the complete interruption/publication experience or writing quality; no owner taste review or wall-clock return rehearsal has been demonstrated.
