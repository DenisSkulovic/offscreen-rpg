...
| Private drafts | Working: create, save, list and reopen; PostgreSQL ownership/revision checks and two-tab browser test. | Three text fields; saved drafts link to a scripted playable opening candidate. Shared setup absent. [Creation](story-creation.md). |
| Opening preparation and request/result storage | Connected scripted flow: request a deterministic playable candidate from a saved draft, save/reopen it and identify stale results. Protected endpoints omit internal prompts and option intentions. Generic processing lifecycle remains independently tested. | The sample is explicitly not adapted to the premise. Background completion through Temporal; no provider adapter; Start from the candidate is unimplemented. [Lifecycle](technical/story-lifecycle.md), [AI runtime](technical/storyteller-runtime.md). |
...
The chamber is useful integration equipment, not an investor-facing POC or the next product milestone. It proves real persistence and execution with authored inputs. A saved premise can now produce a persisted playable opening candidate, including retained option intentions, while the public preview shows only scene prose and choice labels. Starting a live story from that accepted candidate is still unimplemented, so the chamber continues to initialize an unrelated authored scenario. More fixtures or a longer timer would not connect those flows.

The current choice resolver branches on fixture source, narrative revision and option ID. It cannot interpret a newly generated offer. The AI package's offline playable-proposal component is connected through draft candidate persistence, not through story Start, option resolution or the chamber. Continuing memory retrieval and generation-to-commit integration remain absent. These are application gaps, not problems that buying a model call would solve.
...
| --- | --- |
| 1. Define and connect playable creation | A saved draft can request, persist and review a playable opening candidate, including server-only intentions. Join that accepted candidate and explicit Start into one owned story. Preserve input identity and reject stale candidates or duplicate starts. |
...
The immediate next task is to start a live story from the exact accepted playable opening candidate, retaining the stored intentions rather than adding fixture-specific resolver branches. Follow with durable pending resolution and stale-result checks. No live-provider dependency is needed.
