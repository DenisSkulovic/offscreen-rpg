...
The repository structure has now been reviewed against its actual dependency graph. The first four phases of the [monorepo architecture rework](features/2026-09-18--17-58--monorepo-architecture-rework/FEATURE.md) establish `@offscreen/game` as the framework-free rules owner, organize the Storyteller and application packages around honest capabilities, and move Chamber/integration orchestration into tooling workspaces. API and worker compile independently; the API build graph no longer includes worker or workflows. The remaining phases thin the Next.js route tree and align residual tooling/documentation. This remains a bounded reorganization toward the playable POC.

The reorganized API, worker, Chamber and integration workspaces compile, and both Chamber cleanup tests pass. A Chamber smoke reached the API and Temporal worker but could not launch the browser because the existing web production build stops on four strict optional/null errors in `opening-preview.tsx`. Those unrelated POC errors remain visible rather than being folded into the architecture rework.
