...
- Current phase and exact next action: C2 now has the multi-call accounting seam, the `ask_memory`/`read_memory` facade and a durable model-attempt/request/response lifecycle per round. Next make dispatch review and accounting consume that exact identity and packet before connecting provider transport.
