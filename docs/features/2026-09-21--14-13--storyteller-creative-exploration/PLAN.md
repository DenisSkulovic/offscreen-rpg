...
- Current phase and exact next action: C2 now has both the multi-call accounting seam and implemented `ask_memory`/`read_memory` model facade. Next persist a deterministic attempt identity beside each pending exploration request and make dispatch review describe that exact per-round packet before connecting provider transport.
