...
- Current phase and exact next action: C2 now has the accounting seam needed for multiple calls beneath one operation envelope. Next persist a deterministic attempt identity beside each pending exploration request and make dispatch review describe that exact per-round packet before connecting provider transport.
