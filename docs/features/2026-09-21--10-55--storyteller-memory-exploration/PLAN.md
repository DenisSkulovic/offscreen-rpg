...
- Current phase and exact next action: X2; dispatch review now accepts the persisted round attempt ID and exact request. Connect reservation/settlement and ambiguous response persistence to that same identity, then add the single budget-sharing invalid-final repair path before normal provider connection.
- Base/reviewed Git revision and relevant uncommitted changes: base `71c9769`; each scripted model round persists a UUID attempt ID, exact captured request/hash and raw response in the generation-owned exploration artifact before interpretation. Dispatch review rows and append-only decisions are now attempt-owned rather than generation-singleton, while the existing one-shot runtime uses its generation attempt ID. The conversational `ask_memory`/`read_memory` contract remains unchanged.
- Actual checks/results for this revision: database, contracts, application, API and Chamber builds pass; the regenerated disposable baseline applies and the focused held-packet/release case passes 1/1 with fake transport. The preceding crash/replay controller case also passes while asserting that every source call sees its already-persisted attempt ID and exact request. No provider call occurred.
