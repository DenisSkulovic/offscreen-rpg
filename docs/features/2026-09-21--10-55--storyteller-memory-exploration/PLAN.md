...
- Current phase and exact next action: X3 structural evidence is complete. Live memory evaluation is temporarily dependent on the conservative evaluation feature's provider-output compatibility slice; its ordinary-JSON mode removed placeholder collapse but still failed opening topology, so first test the corrected complete `storyteller.v6` opening example with at most one verified-free call, then extend a successful boundary to the conventional and abstract memory questions.
...
- Unresolved findings/blockers: the first facade maps evidence questions to lexical search and possibility questions to one broad-discovery route; adaptive routing remains later evaluated work. A crash after a canonical read but before snapshot commit may recompute the same read-only lookup without double-counting it. The deterministic providers prove mechanics, not cheap-model comprehension or taste. The previously noted replay mismatch did not reproduce in the exact isolated controller case; the broad run is classified as harness/workflow interference unless a narrow reproduction proves otherwise. The first post-X3 free opening still failed local validation, so spending another live call before fixing the shared output boundary would not test memory.
- Provider spend and accounting certainty: two verified-free OpenRouter calls occurred in the shared live-evaluation prerequisite. Both settled at zero microusd with matched ledger calculations and unchanged before/after provider credit snapshots; no memory-model call occurred.
