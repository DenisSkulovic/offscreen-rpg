...
Status: bounded linear slice implemented. A player can accept two to six distinct process plans from one current authored offer, choose a tick horizon and cancel only pending continuation without stopping current work. The durable plan owns successor admission; a quiet/report boundary freezes its result, reprojects authority, and atomically starts the exact still-authorized successor, records a readable block, or stops at the horizon. Broader editing and scene-to-chain re-entry remain.

Nearest extension: accept up to six distinct linear entries from the same current offer and require a relative tick horizon at acceptance. Persist its absolute world tick. The horizon governs **successor admission**: already-running work may finish its admitted boundary, but no next entry starts at or beyond the horizon. Remaining entries become visibly cancelled under a `horizon-reached` terminal plan rather than being silently dropped. This is a bounded itinerary stop, not a promise to pre-empt arbitrary work exactly on the wall-clock instant.
...
- Current phase: U2a, core U2b and bounded U2c linear chains are implemented. Browser/API authoring accepts up to six total entries with an explicit tick horizon; deterministic evidence covers three quiet entries, horizon stopping, pending-only cancellation and prerequisite blocking. Next connect a chain through an actual controlling scene and explicit Storyteller handoff without allowing old permission to bypass that scene. Later delegated-fallback phases remain distinct.
...
- Verification: affected application/web/integration builds pass. Focused PostgreSQL tests prove three quiet entries with no new generation/rolls, horizon stopping without a successor row, pending-only cancellation, and changed-prerequisite blocking. Chamber now exposes the plan revision/cursor, absolute horizon, entry/activity bindings, stop reason and current activity authorization for manual evidence. The earlier full PostgreSQL/Temporal/browser Storyteller suite remains 13/13; the full suite with these new cases has not been rerun after the report-capture ordering fix. No manual browser chain run has occurred.
