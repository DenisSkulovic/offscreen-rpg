...
- Current phase: bounded U2a and situations S2 are implemented. U2b has a strict report-only task/output contract plus report-policy admission, source-bound durable hook storage and a factual/history projection that does not replace current gameplay authority. Implement report-task dispatch and historical publication before calling generated reports playable. First proof still holds at interactions without unattended decisions; later chain/fallback phases remain distinct.
- Implemented boundary: completion follow-up distinguishes quiet, report-only and controlling-scene behavior. Report-only completion preserves the freshly fenced offer, records one hook for the exact activity revision/source passage/tick, and appears separately from current passages and choices. No task is dispatched yet. Switching work continues to rebind retained exact-instance resume authority atomically.
- Verification: game, contracts, database, application and web builds pass for the hook slice. The regenerated baseline contains 32 tables and a freshly reset local database applied exactly one migration. Earlier focused PostgreSQL evidence proves quiet B→A resumption, finite repeat cycles and boundary blocking; no browser/report publication run has occurred.
