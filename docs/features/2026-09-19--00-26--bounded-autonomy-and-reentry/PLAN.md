...
Status: implemented for the bounded standalone proof with S2. A captured activity selects `quiet` or `scene` completion follow-up. Quiet completion appends its factual mechanical passage, creates no consequence/generation intent and reprojects remaining authorized plans. The nonhuman fixture independently selects and completes the same finite repeatable routine twice as distinct instances with zero generation tasks; its third projection is absent. Boundary-rechecked prerequisites can block retained work without granting another attempt or reward.
...
Status: bounded linear slice and explicit scene re-entry are implemented. A player can accept two to six distinct process plans from one current authored offer, choose a tick horizon and cancel only pending continuation without stopping current work. The durable plan owns successor admission; a quiet/report boundary freezes its result, reprojects authority, and atomically starts the exact still-authorized successor, records a readable block, or stops at the horizon. A controlling scene blocks the next entry. The Storyteller can receive and re-offer that exact private entry, but publication alone does not restart it: the player's ordinary selection explicitly rebinds the accepted plan. Broader editing remains.
...
- Current phase: U2a, core U2b and bounded U2c linear chains with scene re-entry are implemented. Browser/API authoring accepts up to six total entries with an explicit tick horizon; deterministic evidence covers quiet continuation, horizon stopping, pending-only cancellation, prerequisite blocking and a controlling scene that requires an explicit Storyteller handoff plus player selection. Later delegated-fallback phases remain distinct.
- Implemented boundary: the accepted plan stores exact authored process terms and entry identities. At completion, report evidence freezes before current continuation changes; the queue alone consumes one successor after fresh authorization, exact-term, prerequisite and occurrence checks. A scene exposes only the pending private commitment to the Storyteller, remains blocked when a matching offer publishes, and rebinds only when the player selects that exact offer. Failure leaves the plan blocked and visible rather than inventing replacement work.
- Verification: affected application/web/integration builds pass. Focused PostgreSQL tests prove three quiet entries with no new generation/rolls, horizon stopping without a successor row, pending-only cancellation, changed-prerequisite blocking, and explicit scene re-entry. Chamber exposes the plan revision/cursor, absolute horizon, entry/activity bindings, stop reason and current activity authorization for manual evidence. The earlier full PostgreSQL/Temporal/browser Storyteller suite remains 13/13; the full suite with these new cases has not been rerun. No manual browser chain run has occurred.
