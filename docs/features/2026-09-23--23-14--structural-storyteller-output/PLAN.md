...
## Phase S7 — Fresh live proof

- Question: does a fresh Sol/OpenAI opening using the repaired v11 projection pass provider strict-schema transport and semantic admission without zero deltas, unavailable prerequisites or invented references?
- Fixed variables: Seyda mechanical opening, Quiet Eerie Mystery profile, reasoning disabled, native strict JSON schema, one primary call, no tools/repair/retry/fallback. The inspected packet was 24,180 serialized bytes with a 16,860-byte output schema; the finite operation ceiling was 150,000 microusd.
- Result: generation `76d3839b-271e-4e14-802c-9073765cc820` succeeded and settled in one attempt. It returned five admitted plans, including the exact authorized warehouse process, and no unavailable quantity prerequisite. Usage was 3,591 prompt, 882 completion, zero reasoning, 3,588 cache-write and 4,473 total tokens. Charged and calculated cost matched at 17,796 microusd; the operation is complete and account/run/operation reservations are all zero.
- Qualitative reading: the opening strongly follows Quiet Eerie Mystery—ordinary release, concrete sensory detail, one perceivable crate discrepancy, and distinct investigation/engagement/withdrawal intentions. It also exposes two non-structural follow-ups: the authorized directions plan carried an implausible five-second fixture duration, the model estimated thirty minutes for a close inspection after seeing no world-scale guidance beyond the thirty-minute shift, and leaving Seyda was narrated without a typed location effect. These are content/time-semantics and outcome/state-coherence findings, not reasons to alter creative temperature.
- Accounting note: the immediate and delayed OpenRouter credits endpoint still reported the prior cumulative USD 0.657259296 at 2026-09-23T21:36:55Z, while the durable provider response and local calculation matched. No further provider call ran; treat account-level reporting as lagging.
- Verdict: the repaired structural contract passes this one live sample. One success is not a reliability rate. Status: complete.

## Current checkpoint

- Current phase and exact next action: retain S7 as the completed structural proof. Fixed-second T4 owns the diagnosed fixture/world timing changes; typed outcome/state coherence remains a separate admission-design problem. Do not spend again merely to resample either issue.
- Base/reviewed Git revision and relevant uncommitted changes: S1/S2 are pushed at `378292a`; opening state-indexing at `5a58a68`; the typed-empty repair at `6cb98ad`; consequence state-indexing and durable guidance at `8aec5f9`; narrative retrieval projection at `7c0b220`; S7 evidence is the current documentation change.
- Actual checks/results for this revision: Storyteller builds and passes 40/40 tests. Provider-free audits remain 22,071 request / 16,653 schema bytes for the mechanical opening and 32,581 / 23,057 for the activity consequence. The released live packet was 24,180 / 16,860 bytes and succeeded with a valid admitted result.
- Unresolved findings/blockers: consequence envelope compaction remains a separate candidate because continuity, document changes, evidence and activity resumption need a deliberate boundary. Typed-state changes narrated without matching effects remain semantically undetectable by the current schema. One generated sample does not establish v11 reliability.
- Provider spend and accounting certainty: S7 settled and reconciled at 17,796 microusd (3,591 prompt, 882 completion, zero reasoning), with zero remaining persistent reservation and the funding/run disabled after the one attempt. Known local-calendar 2026-09-24 development inference is USD 0.017796. The OpenRouter account endpoint still reported the prior USD 0.657259296 cumulative usage and USD 9.342740704 remaining at 2026-09-23T21:36:55Z, so account reporting is lagging and no further provider work ran.
