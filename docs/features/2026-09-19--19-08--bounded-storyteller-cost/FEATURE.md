...
Free, paid-tier and explicitly funded on-demand profiles use one server-side policy system. The effective context may be much smaller than the model supports. User-selected lower limits, task caps, usage windows and platform funding remain authoritative even for a high-tier account. Commercial prices and payment integration are not prerequisites for exercising this safely offline.
...
A synthetic account uses its last window allocation while a required turn is pending. The story holds at that boundary with progress intact and explains the limiting resource and recovery time when known. After the window recovers, it becomes eligible; explicit Resume is the default and manual pause always survives. A different account profile can permit larger context or another allowlisted model without resetting already consumed usage. Funding exhaustion has no fabricated reset countdown, and quota exhaustion cannot silently enable on-demand charges.
...
9. A tier's input/output/model limits apply below provider capacity; lower personal/story caps still win. Missing or invalid policy fails closed. Profile edits and resets never replenish development funding.
...
[Usage policy](../../technical/usage-policy.md) owns account entitlement composition, windows, funding distinctions, recovery and the conservative initial preset. [Context and cost](../../technical/context-and-cost.md#bounded-work-not-an-open-ended-agent) owns recipes, operation accounting and context tradeoffs. [Player experience](../../player-experience.md#acting-and-waiting) owns the visible stop/defer promise. [Memory](../2026-09-18--20-09--storyteller-memory-and-recall/PLAN.md) consumes the limits. [Live evaluation](../2026-09-18--17-27--conservative-live-model-evaluation/FEATURE.md) owns the later authorization boundary. [PLAN.md](PLAN.md) is the implementation handoff.
