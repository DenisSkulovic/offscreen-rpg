...
- Phase: B1 complete/B2 implementing. Effective-policy admission, task envelopes, rich attempt audit, sanitized inspection, durable window allocations, operation-wide aggregate ceilings and fail-closed dispatch-time authority recheck are implemented. Exact next action: limiting-reason projection and player-visible hold/recovery. Do not enable provider execution. Storage C1/C2 remains independently ready.
- Slice base: `ebad5dd`; dispatch-authority application/test/documentation changes are uncommitted.
- Verification: application and API-integration workspaces compile; the focused fake-provider accounting probe proves missing authority releases the allocation and performs zero transport calls. No browser or live calls.
