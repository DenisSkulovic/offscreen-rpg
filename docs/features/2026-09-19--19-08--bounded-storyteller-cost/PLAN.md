...
2. Make explicit Retry allocate a fresh attempt beneath the existing generation operation, build a complete repair request from the immutable task/candidate/findings, recheck current dispatch authority, and settle it through the ordinary ledger. The repair returns a complete task result and uses unchanged publication fences. Crash/replay cannot redispatch a delivered round.
3. Project Retry only for eligible remaining repair capacity and add fake-provider connected evidence: initial invalid candidate, preserved receipt/time, explicit repair, one valid publication, two settled attempts, no duplicated mechanics; plus one-shot, exhausted, refusal, uncertainty and stale-story contrasts.
...
- Phase: B1 complete/B2c phase 1 implemented, phase 2 next. The owner approved this scope on 2026-09-23 after the fresh Seyda diagnostic produced a useful but invalid settled candidate. Exact next action: make explicit Retry build/persist a bounded complete-result repair request and allocate its attempt beneath the existing operation; do not expose Retry until restart/idempotency and publication are proven provider-free.
- Existing recovery baseline: fake-provider story resolutions prove that pre-transport authority denial releases an `unsent` attempt and permits the same admitted intention to resume, while settled invalid output is non-retryable under `single-turn.v1` and uncertain delivery remains operator-blocked with retained liability and a stopped account. B2c adds a separate bounded repair path only for newly captured repairable tasks.
- Verification: Storyteller 39/39 and focused usage-policy 5/5 pass; Storyteller, database, application and API-integration packages build with the repairable resource/capture contract. The database baseline was updated but not reset in this slice. Existing focused mechanical tests prove generation/decision ownership, failure retention, publication transfer, selection consumption and no held-time catch-up. All providers were fake; no live calls.
...
- Spend: B2c implementation is provider-free. The motivating live diagnostic is fully settled and brought 2026-09-23 development inference to USD 0.2682458; no repair call is authorized until the provider-free path and exact packet are inspected.
