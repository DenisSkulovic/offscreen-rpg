...
- Current phase and exact next action: T1 and the provider-free T3 path are complete, but forensic review found that the live opening path never receives T3's authored executable plan. It receives only opening prose and recreates a new plan under model control. Replace that reconstruction with an exact authorized plan reference before changing prompts or profiles. Provider error reconciliation remains a separate transport concern. T2 must also remove obsolete tick terminology from current model packets and player-facing time, not merely rename internal storage.
