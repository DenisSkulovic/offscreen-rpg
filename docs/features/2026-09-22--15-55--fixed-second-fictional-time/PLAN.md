...
- Current phase and exact next action: authorized opening references, recovery fixes, fictional-second labels, separation of routine progress from narrative history, and the progressive identity/relationship contract are implemented. Prompt v8 tells the model exactly when later correctness warrants promotion without extracting every noun or imposing social drama. The maintained return benchmark proves a named identity and source-linked relationship cross unrelated material and appear as current records in a later packet. Internal tick field names remain; complete T2 next, then inspect one held Sol packet. A billed invalid candidate still has no extra repair round. Do not tune profiles yet.
- Base/reviewed Git revision and relevant uncommitted changes: HEAD is `16108d2`. Prompt v8 and the expanded return benchmark are committed there; routine-history separation is committed at `3c82ada`. The current working change records completed verification only.
- Actual checks/results for this revision: all 39 Storyteller tests passed, including the transmitted materialization instruction and 48-KiB request bound. The API-integration workspace build passed. After freeing regenerable workspace caches and applying the repository's guarded Docker stale-socket repair, focused database cases passed for routine-progress passage separation and the conventional identity/relationship return. No provider calls.
