...
- Current phase and exact next action: T1 and the provider-free T3 path are complete. A bounded Sol run now proves the corrected live opening and paid shift; next fix or explicitly support reconciliation of OpenRouter's HTTP-200 embedded `provider_unavailable` envelope before continuing this story or running the contrast profile. T2's broad internal coordinate rename follows separately; its current names are misleading but no longer change units or behavior.
...
- Live evidence: Sol/Character-Driven Drama story `a15c682c-9a30-4d92-b966-a2ed8d551299` opened with the exact thirty-minute/six-septim offer plus four materially distinct alternatives and 60/120/600-second estimates. The shift admitted as a 1,800-second clock wait, completed once, paid once and produced a grounded post-shift scene plus three subsequent social turns. Successful new calls cost USD 0.125934. Two separately identified overload envelopes were confirmed absent from OpenRouter generation metadata and cumulative billing, then reconciled as unsent with zero reservation. OpenRouter cumulative usage is USD 0.286277596. The second failed turn remains explicitly retryable; the server is stopped and no contrast run started.
...
- Provider spend and accounting certainty: successful calls in this live phase cost USD 0.125934. Latest independent cumulative OpenRouter usage is USD 0.286277596; local reservations are zero and both embedded overload attempts were reconciled against provider generation metadata and cumulative billing.
