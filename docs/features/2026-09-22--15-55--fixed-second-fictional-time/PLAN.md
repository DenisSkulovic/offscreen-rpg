...
- Current phase and exact next action: T1 and the provider-free T3 path are complete. The matched Sol/Luna run shows that the fixed-time and reward mechanics hold when the model selects the intended process, while plan-shape validation still permits a model to turn the same bounded job into a full-duration finite action followed by another process. Diagnose that admission boundary before changing prompts or profiles. Provider error reconciliation remains a separate transport concern. T2's broad internal coordinate rename follows separately.
- Base/reviewed Git revision and relevant uncommitted changes: T1 is committed at `eb198af`; T3 is committed at `e706a6d`; headless activity control is committed at `ed62a85`. The current slice extends the same headless control command to finite action executions because both are exposed as timed work in play.
- Actual checks/results for this revision: game build plus 34 tests passed; Storyteller build plus 37 tests passed; application build plus 15 tests passed; contracts, web production and API-integration builds passed. Provider-free focused integration passed finite-action pause/speed, general clock-wait completion and the Seyda 1,800-second warehouse completion. The current headless script change has only a syntax check planned because it is a narrow operator tool change.
...
- Interpretation: Sol is materially stronger at multi-turn character continuity and at choosing the intended mechanical shape. Luna's prose and options can be good, so the result is not simply weak writing. Its output-limit padding, duplicate-duration plan and later invalid candidate are model-route reliability defects. The system also owns part of the failure because admission allowed a selected warehouse activity to become a full-duration finite action followed by the warehouse process, violating the authored bounded commitment. Preserve these traces and fix the plan/admission invariant before prompt tuning.
