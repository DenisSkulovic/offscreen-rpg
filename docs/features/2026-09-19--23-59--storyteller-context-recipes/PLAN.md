...
- Exact next action: continue R2 in application admission: load a supplied scene range under the story lock and propagate/update that scope durably across publication. The pure `active-scene.v1` contract and `bounded-scene.v2` selector now require complete in-range evidence, exact current boundary and explicit required identities; the fifteen-turn human/nonhuman audit fixtures exercise that contract.
- Known gaps: normal application admission does not yet supply or persist active-scene scope, so unscoped stories still use six optional recent passages. Current prose remains duplicated and note sources remain mandatory raw input; there is no tool/multi-round runtime. Opening-note removal needs semantic review before optimizing further.
