...
- Exact next action: continue R2 with an explicit publication-owned scope replacement contract so a genuinely new situation can begin a fresh anchor without automatic passage-count inference. New profiled stories now persist `active-scene-anchor.v1`; continuation, consequence and report admission load its complete committed range under the story lock and derive the exact current boundary. The pure selector still owns completeness and overflow rejection.
- Known gaps: existing/unprofiled stories without an anchor still use six optional recent passages. A profiled scope currently starts at passage one and has no explicit reset, so more than forty passages holds instead of truncating. Current prose remains duplicated and note sources remain mandatory raw input; there is no tool/multi-round runtime. Opening-note removal needs semantic review before optimizing further.
- Verification: the existing 30 Storyteller tests passed at the earlier checkpoint but were not rerun. The scoped selector passed focused Storyteller typecheck; after durable integration, DB/contracts builds and the focused application typecheck pass. No migration, command, runtime or broad suite was executed. Authored captures do not prove multi-purpose HTTP gameplay.
