...
- Exact next action: finish the R2 failure/restart evidence and provenance/current-body coordination with memory Phase 1; then run the deferred packet audit/tests as one later batch. Continuation and consequence output can now propose `restart-at-current`, but only successful revision-fenced publication atomically replaces the anchor with its newly committed passage. Omission/continue preserves it; reports/openings cannot reset it.
- Known gaps: existing/unprofiled stories without an anchor still use six optional recent passages. Current prose remains duplicated and note sources remain mandatory raw input; provenance/body separation is coordinated with memory Phase 1. There is no automatic scene-boundary inference, tool/multi-round runtime or paid condensation. Opening-note removal needs semantic review before optimizing further.
- Verification: the existing 30 Storyteller tests passed at the earlier checkpoint but were not rerun. After the scope/replacement changes, the Storyteller build and focused application typecheck pass; earlier DB/contracts builds also pass. No migration, command, runtime or broad suite was executed. Authored captures do not prove multi-purpose HTTP gameplay.
