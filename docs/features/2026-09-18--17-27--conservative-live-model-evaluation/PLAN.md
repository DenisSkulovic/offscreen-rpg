...
Exact next action: stop cycling free models under the same native-schema assumption. Design and dry-run one provider-neutral compatibility boundary that can request ordinary JSON without schema-constrained placeholder collapse, then apply the existing local validator and fail closed without an automatic repair. Compare its exact packet with the current native structured packet before authorizing another call. Memory retrieval remains out of the live launcher until this one-round output boundary produces a usable opening.
