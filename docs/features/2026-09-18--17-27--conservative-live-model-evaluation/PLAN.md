...
Exact next action: use a fresh verified-free run identity to test the tightened schema. If the opening validates, let the existing short-loop launcher Start it and capture the first real continuation packet. If another advertised structured-output endpoint violates the schema, treat free native structured decoding as unreliable and consider a separately designed local deterministic extraction/validation boundary rather than cycling models.
