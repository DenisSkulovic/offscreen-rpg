...
- Exact next action after this slice: extend the same stable-link principle to already-existing records that become current without being revised, using task-local catalogue handles resolved during restart publication. Dynamic mechanical item creation and current inventory-file projection remain a separate typed-state slice.
- QA maintenance: reusable-start-package v10 proves the corrected thread returns through a restarted active-scene stable cue rather than a repeated choice hint, then applies the shared conventional/abstract oracle and lexical discovery checks.
- Verification: affected builds and the four-case disposable start-package integration pass with both oracle contrasts, active-scene stable-ID recall, create, unrelated play, revision, lexical discovery and exact current-version loading. The focused application publication test passes for source-backed place/identity creation, same-kind revision, stale-revision rejection and kind-confusion rejection. Storyteller and application builds pass after the shared exact-read priority audit. The integration harness now permits a bounded 30-second production-web readiness window after its former 10-second limit caused a false startup failure on the supported slow laptop. No provider call was made.
