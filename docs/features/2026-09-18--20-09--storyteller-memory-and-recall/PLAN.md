...
- Exact next action after this slice: admit useful identity/place/thread links in the generic turn state that owns them, then feed those IDs into the resolver on ordinary continuation construction. Start with one existing stable owner and no inferred entity extraction or extra model round. Dynamic mechanical item creation and current inventory-file projection remain a separate typed-state slice.
- QA maintenance: reusable-start-package v9 adds structured stable-ID cue resolution and reason traces to the shared conventional/abstract retrieval oracle before current-root lexical discovery.
- Verification: affected builds and the four-case disposable start-package integration pass with both oracle contrasts, stable-ID cue resolution, create, unrelated play, revision, lexical discovery and exact current-version loading. The later focused application build and publication test also pass for source-backed place/identity creation, same-kind revision, stale-revision rejection and kind-confusion rejection. No provider call was made.
