...
- Exact next action after this slice: evolve the maintained return benchmark from immediate handoff to a later return across enough irrelevant documents/passages to prove the promoted record remains retrievable and current-state corrections outrank old narrative evidence.
- QA maintenance: reusable-start-package v3 covers exact choice-to-world-section handoff; extend the return journey with identity/source evidence when Phase 2 lands.
- Verification: affected builds, the 33 Storyteller unit tests and the four-case disposable start-package integration pass with both existing-document and same-result promotion handoffs. No provider call was made.
