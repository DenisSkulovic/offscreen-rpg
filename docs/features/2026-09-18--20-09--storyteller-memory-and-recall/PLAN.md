...
- Exact next action after this slice: connect the proven search contract to the bounded `needs_context` exploration round or structured identity/thread cues, preserving one shared operation budget and current-root fence.
- QA maintenance: reusable-start-package v7 adds current-root lexical discovery, privacy, supersession and exact-load evidence to the existing handoff journey.
- Verification: affected builds and the four-case disposable start-package integration pass with create, unrelated-play, revise, lexical discovery and later exact current-version loading. No provider call was made.
