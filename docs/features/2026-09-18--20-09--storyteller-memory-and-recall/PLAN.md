...
- Exact next action after this slice: admit useful identity/place/thread links in the generic turn state that owns them, then feed those IDs into the resolver on ordinary continuation construction. Start with one existing stable owner and no inferred entity extraction or extra model round.
...
- Verification: affected builds and the four-case disposable start-package integration pass with both oracle contrasts, stable-ID cue resolution, create, unrelated play, revision, lexical discovery and exact current-version loading. No provider call was made.
