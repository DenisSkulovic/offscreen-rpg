...
- Exact next action after this slice: use the same stable selection boundary in the maintained return benchmark for one promoted identity/episode record with original-source provenance. Decide the smallest promotion shape from actual fixture needs; do not create a universal NPC/location graph or jump to semantic search first.
...
- Verification: affected builds, the 33 Storyteller unit tests and the four-case disposable start-package integration pass. No provider call was made.
