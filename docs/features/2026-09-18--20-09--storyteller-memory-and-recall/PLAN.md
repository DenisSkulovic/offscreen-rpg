...
- Exact next action after this slice: define a bounded return benchmark that is not hinted by the immediately preceding choice. Establish exact identity/alias/thread leads and a measured lexical baseline over current canonical versions and source evidence before evaluating semantic retrieval.
- QA maintenance: reusable-start-package v6 covers exact world/existing-document handoff, same-result promotion, intervening play and current-version supersession with source evidence.
- Verification: affected builds and the four-case disposable start-package integration pass with create, unrelated-play, revise and later-current-version handoffs. No provider call was made.
- Remaining choices: linked record shapes and promotion boundary, numerical tuning, and Phase 5 local model/backend selection. Recent passages plus continuity notes remain the only general lifetime narrative memory.
