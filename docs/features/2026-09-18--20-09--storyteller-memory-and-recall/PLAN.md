...
- Exact next action after this slice: define and implement the typed authoritative path for dynamic item creation and current inventory projection so new possessions are not descriptive-document hacks and `state/story-items.json` does not remain a bootstrap-only snapshot.
- QA maintenance: reusable-start-package v11 proves the corrected thread and unchanged quay return through a restarted active-scene stable cue, then applies the shared conventional/abstract oracle and lexical discovery checks.
- Verification: 12 affected builds and the four-case disposable start-package integration pass with both oracle contrasts, changed/existing active-scene stable-ID recall, create, unrelated play, revision, lexical discovery and exact current-version loading. The focused application publication test passes for source-backed place/identity creation, same-kind revision, stale-revision rejection and kind-confusion rejection. The integration harness permits a bounded 30-second production-web readiness window after its former 10-second limit caused a false startup failure on the supported slow laptop. No provider call was made.
