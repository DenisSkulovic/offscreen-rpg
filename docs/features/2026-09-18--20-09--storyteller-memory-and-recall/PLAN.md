...
- Exact next action after this slice: resolve admitted current identity/place/thread cues into stable document candidates and an inspectable reason trace before lexical search. Replace fixture paths with runtime stable IDs at the application boundary; do not parse arbitrary player prose or add a model round yet.
- QA maintenance: reusable-start-package v8 adds the shared conventional/abstract retrieval oracle and abstention evidence to current-root lexical discovery.
- Verification: affected builds and the four-case disposable start-package integration pass with both oracle contrasts, create, unrelated play, revision, lexical discovery and exact current-version loading. No provider call was made.
