...
- Inspection findings: the profile contributes 1,045 useful bytes; empty opening context is small. The opening-specific provider result now omits continuity/arrival machinery while normalization preserves the stable stored publication shape. The same premise/profile packet fell from 7,097 to 4,303 bytes: system instructions 1,647 → 1,153, output schema 3,663 → 1,367, and user context remained exactly 1,358. The API-only probe verifies `awaiting-review` and zero provider attempts without starting Next.js or Chromium. More importantly, arbitrary-premise openings can offer narrative choices but cannot author the richer activity/action-plan opportunity contract currently limited to authored mechanical seeds. The base prompt correctly withholds mechanical authority and does not need a copied or externally referenced D&D rulebook, but it underspecifies the game's deliberate-time/activity rhythm.
- Next: decide how generated openings author mechanically admissible opportunities without granting the model authority to invent rules, and add reusable packet comparison/audit output rather than relying on manual JSON arithmetic. Do not expose release until a priced route, funding preflight and explicit one-call authorization are deliberately configured.
- Immediate handoff: [context recipes R1](../2026-09-19--23-59--storyteller-context-recipes/PLAN.md) owns the reusable multi-purpose audit. Opening-note removal needs semantic review: the original validator accepted `current` as evidence, so empty previous history did not require removing all opening notes. Byte savings alone do not establish equivalent continuity. Resume this feature's remaining controls after that audit; do not create a second comparison tool here.
