...
Phase 2 has started with a pure structured proposal validator. It reports bounded diagnostics for shape, captured evidence, declared prerequisites/effects, unavailable abilities, undeclared skills and unsupported situational modifiers. Fixture admission exercises the same policy. Every sheet retains the selected D&D ability scores while its current form declares which abilities and skills apply; the microbe and conventional fantasy character therefore use the same rules without pretending to have identical affordances.
