...
Direct exactly-once adjudication, bounded emergent state and the complete offline DM-turn consequence contract are implemented. Immediate selection saves one receipt and durable follow-up without time/activity adaptation. The consequence task now returns scene prose plus zero to four fresh `immediate-action.v1` plans rather than selecting IDs from the old authored offer. Storyteller validation checks captured evidence and state; application publication repeats deterministic admission and atomically stores the new passage, public offer and private plans. The scripted source creates a fresh plan through this same path. Next: connect the planning contract to mechanical opening review and exercise consecutive browser rounds. Capability-changing transformations remain separate admitted state changes. Scope later trace work to the same DM-turn lifecycle. No schema compatibility layer is required; discarded pre-POC artifacts are reset.
