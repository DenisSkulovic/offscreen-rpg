...
Execution scope: the owner's time/choice clarification selects the product direction and authorizes these phases. T1/T2 are implemented; T3 follows its calendar/world-boundary dependency. Do not dispatch another agent. No provider execution.
...
Status: implemented. Automatic/check plans require bounded positive `durationTicks`; missing duration fails validation. Public offers expose the admitted tick duration. Selection saves a dedicated finite execution, gives it the campaign's single advancing slot, and defers rolls, effects and receipts until the exact target boundary. Finite executions support replay-safe pause, resume and remaining-time pace changes; each transition is durable history, and workers recheck the execution after the story lock so an old wake cannot overtake a pause. Settlement then acquires the required-turn hold and requests sequential narration, so provider latency adds no fictional time. Process/resume choices remain separately labelled extended work. Focused rate/pause/retime/resume, interrupted A → timed response → retained A and worker-restart/replay probes pass.
...
Generic temporal opportunity rules remain a separate rule-family extension, not T3 scope. T3 eligibility must nevertheless fail closed whenever an admitted availability window, recurring phase, required cadence/pace term, deadline or other boundary can change the action before its target. The Seyda Neen warehouse-shift benchmark exercises this distinction without introducing employment/daylight policy into shared code. Storyteller-authored terms become authority only after typed validation; arbitrary prose or model callbacks are never overlap fences.
...
- Phase: T3b deterministic evidence on base `0a24b5f`. T3a and the first T3b visibility/recovery slice are implemented. The public resolution contract distinguishes pending-action evidence from committed evidence; active execution copy explicitly says no outcome/roll/effect is current, polling continues even after early preparation failure, and retry is withheld until settlement. After settlement the existing receipt state and same-generation recovery remain authoritative. Ineligible actions preserve committed-consequence semantics and sequential preparation.
- Exact next slice: add Chamber delay/release/failure controls for the planned `finite-action-overlap.v1` journey. Exercise early, late, pause, restart, failed preparation and due-boundary fallback without exposing the pending outcome. Recovery must reuse the saved generation/result and never redraw, duplicate the operation allowance or clear a manual pause. Promote the case from planned only when each ordering is repeatable through application controls rather than sleeps or database edits.
- Reviewed base: `0a24b5f`; the T3a implementation and Docker-independent disposable test reset are committed and pushed.
- Evidence: game, database, Storyteller and application builds pass; the full affected eleven-package integration graph and production web build pass. The focused game policy test passes 14/14 and Storyteller task tests pass 17/17, including the pending task's v9 contract. After applying the updated baseline to a fresh disposable database, the provider-free worker-restart connected journey passes with one pending generation promoted to one receipt/publication and no sequential consequence intent. The reusable test reset now connects directly to loopback PostgreSQL, avoiding an unrelated Docker named-pipe permission dependency after services are already running. Baseline generation itself stopped and restored its backup after the local process reported insufficient memory, so the single SQL baseline and Drizzle snapshot were updated together manually and then exercised successfully by migration. No provider route was available to these commands.
