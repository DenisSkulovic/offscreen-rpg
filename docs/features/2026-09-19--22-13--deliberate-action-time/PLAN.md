...
Status: active. Automatic/check plans require bounded positive `durationTicks`; missing duration fails validation. Public offers expose the admitted tick duration. Selection saves a dedicated finite execution, gives it the campaign's single advancing slot, and defers rolls, effects and receipts until the exact target boundary. Finite executions support replay-safe pause, resume and remaining-time pace changes; each transition is durable history, and workers recheck the execution after the story lock so an old wake cannot overtake a pause. Settlement then acquires the required-turn hold and requests sequential narration, so provider latency adds no fictional time. Process/resume choices remain separately labelled extended work. Focused rate/pause/retime/resume and interrupted A → timed response → retained A probes pass. Remaining T2 work is an explicit worker-restart/replay probe and final source review before claiming the phase complete.
...
- Phase: T1 complete; T2 active. Positive finite duration is mandatory and public for automatic/check actions. A durable action execution advances through the existing clock and settles mechanics once at its target; no click-time jump and no provider-latency charge. Focused integration proves pause, retime, stale-wake rejection, resume and three complete action → receipt → narration rounds. The beacon trace proves a five-tick encounter response advances world time without donating progress to interrupted repair, after which the exact retained work remains resumable. An explicit worker-restart/replay probe remains before T2 exit. Calendar/world-time K1/K2 remains after T2 and before overlap acceptance for timed stories.
...
- Evidence: the affected eleven-package graph and production web compile. The single baseline regenerates and applies to the disposable database. Focused integration covers finite admission with no premature receipt, pause, a stale worker wake, pace change, resume, exact target settlement, durable lifecycle events, three state-shaped consequences and interrupted-work chronology with preserved contribution. A worker-process restart/replay probe and manual rhythm review remain. No broad suite was rerun in this slice under the owner's deferred-check policy.
