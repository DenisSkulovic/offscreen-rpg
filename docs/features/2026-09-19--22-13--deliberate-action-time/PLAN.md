...
Status: follows K1/K2; required for the finite-action overlap acceptance, not a general speculative agent runner. [Calendar/world-time K1/K2](../2026-09-19--22-29--calendars-and-world-deadlines/PLAN.md) supplies date interpretation and world obligations; integrate K2 before claiming overlap safe in time-limited plots. T1/T2 do not depend on elaborate calendar features.
...
- Phase: T1 and T2 complete. Positive finite duration is mandatory and public for automatic/check actions. A durable action execution advances through the existing clock and settles mechanics once at its target; no click-time jump and no provider-latency charge. Focused integration proves pause, retime, stale-wake rejection, resume, three complete action → receipt → narration rounds, interrupted A → timed response → retained A, and a real worker restart during a five-tick action. T3 follows K1/K2 and must introduce the distinct pending-evidence contract rather than reusing committed consequence input.
- Reviewed base: `f00fb16`; this closure adds focused restart/replay evidence and documentation only. No production or schema change was required.
