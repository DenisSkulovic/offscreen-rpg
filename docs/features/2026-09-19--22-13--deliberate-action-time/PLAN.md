...
- Phase: T3b recovery, visibility and deterministic evidence on base `6eba7bb`. T3a is implemented: closed finite actions with no due world obligation draw once, persist source/projected-state-fenced pending evidence, enqueue one distinct pending-consequence generation and promote that same identity to the settled receipt. Early publication returns privately until receipt; settlement acquires the generation-owned hold and sends an idempotent publication wake. Ineligible actions preserve committed-consequence semantics and sequential preparation.
- Exact next slice: make pending/settled-awaiting-narration states legible in the public snapshot and recovery controls, then add Chamber delay/release/failure controls for the planned `finite-action-overlap.v1` journey. Exercise early, late, pause, restart, failed preparation and due-boundary fallback without exposing the pending outcome. Recovery must reuse the saved generation/result and never redraw, duplicate the operation allowance or clear a manual pause.
- Reviewed base: working implementation after `6eba7bb`; no unrelated changes were present.
