...
- Phase: T3 contract preparation on base `afac93a`. T1/T2 and K1/K2 are complete. Implement the distinct pending-evidence contract and fail-closed overlap eligibility first; do not reuse committed consequence input or treat every fixed-duration action as safe. Intervening world obligations and future admitted temporal conditions make overlap ineligible while preserving the sequential path.
- Reviewed base: `afac93a`; the calendar/world-time feature is closed and its mixed report/controlling-boundary rehearsal passes.
