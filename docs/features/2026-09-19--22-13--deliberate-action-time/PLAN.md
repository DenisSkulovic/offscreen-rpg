...
- Phase: T3a pending-evidence foundation on base `8512724`. T1/T2 and K1/K2 are complete. First add a distinct pending-action task/context contract and a pure fail-closed eligibility decision used by admission. Eligibility requires a closed finite result, stable participants/resources and no due world obligation or admitted temporal fence through the target tick. Ineligible actions retain the current sequential path; committed consequence input remains unchanged.
- Exact next slice: freeze the eligible action's once-drawn resolution and projected end-state identity under the story lock, persist them as private execution evidence, and enqueue at most one preparation operation. Do not apply effects, create history/receipts, expose possessions or acquire the publication hold before settlement. Add the settlement-side validation and promotion seam in the same slice so persisted pending evidence cannot become an unused alternate authority. The following T3b slice will join early/late preparation completion to one receipt-gated publication owner and cover pause/restart/retry races.
- Reviewed base: `8512724`; the calendar/world-time feature is closed, world-report follow-ups and typed temporal-rule direction are durable, and its mixed report/controlling-boundary rehearsal passed on the prior implementation slice.
- Evidence: the provider-free focused wrapper rebuilt the affected eleven-package graph and production web, regenerated the disposable test database, restarted the Temporal worker before the action target, then observed exactly one settled execution, receipt, lifecycle settlement and consequence publication at +5 ticks. Two replayed settlement calls were inert and the published decision held the clock. Existing focused evidence covers controls, stale wakes, exact target settlement, three state-shaped consequences and interrupted-work chronology. Human rhythm/taste remains deliberately unverified; no broad suite or live inference ran.
