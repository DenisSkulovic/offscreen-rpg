...
Status: active. Automatic/check plans require bounded positive `durationTicks`; missing duration fails validation. Public offers expose the admitted tick duration. Selection now saves a dedicated finite execution, gives it the campaign's single advancing slot, and defers rolls, effects and receipts until the exact target boundary. Settlement then acquires the required-turn hold and requests sequential narration, so provider latency adds no fictional time. Process/resume choices remain separately labelled extended work. Remaining T2 work is rate/pause/restart evidence and the interrupted-activity chronology case before claiming the phase complete.
...
- Phase: T1 complete; T2 active. Positive finite duration is mandatory and public for automatic/check actions. A durable action execution advances through the existing clock and settles mechanics once at its target; no click-time jump and no provider-latency charge. Focused instant-pace integration proves three complete action → receipt → narration rounds. Rate/pause/restart and interrupted-work chronology remain before T2 exit. Calendar/world-time K1/K2 remains after T2 and before overlap acceptance for timed stories.
...
- Evidence: source trace of every `projectCampaignClock` caller, due-time projection and action/activity consequence admission; affected packages and production web compile. Game tests pass 25/25. Focused integration covers three finite instant-pace executions with no receipt before settlement, exact target settlement, operation-owned intent before generation transfer and state-shaped consequences. The broad story suite reached 23 passing cases but retains an unrelated generated-passage provenance fixture failure, so it is not claimed green. Manual rhythm, rate/pause/restart and interrupted-work chronology remain.
