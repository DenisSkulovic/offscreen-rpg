...
The proposed [Storyteller creative exploration](2026-09-21--14-13--storyteller-creative-exploration/FEATURE.md) feature addresses a different quality problem: discovering surprising but fitting narrative connections when the Storyteller does not already know the twenty facts to retrieve. It reuses the memory route and later relational/global views, but adds bounded divergent discovery, private direction comparison and fork-based taste evaluation. Its first provider-free benchmark phase begins only after the owner confirms the scope; do not mistake factual retrieval success for this capability.
