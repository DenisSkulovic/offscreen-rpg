...
Next program: long-story memory and retrieval. The owner made this the product-critical implementation priority on 2026-09-21: without bounded, evidence-grounded recall across extreme histories, additional content mechanics do not establish a worthwhile game. [Bounded Storyteller memory exploration](2026-09-21--10-55--storyteller-memory-exploration/FEATURE.md) is the one current delivery slice. Its persisted search/read/final controller and exact packet supervision are implemented; connected live evaluation is stopped by one unresolved pre-header provider attempt. The older [Storyteller memory and situated recall](2026-09-18--20-09--storyteller-memory-and-recall/FEATURE.md) remains the single umbrella for later episodic/source memory and measured semantic or relational experiments.
...
The existing Storyteller memory umbrella is the shared product/authority owner. Do not implement a parallel memory store or second exploration runner from its broad phase wording. Do not issue another provider request while the active exploration checkpoint remains accounting-uncertain; safe deterministic gameplay integration and feature-folder consolidation may continue without weakening that stop.
...
Memory exploration X3 remains the selected program but its next live step is blocked by explicit reconciliation. The item-state checkpoint at `a38076c` admits dynamic items; synchronizing its canonical inventory projection remains a contained storage seam and must not displace the memory program unless it blocks the connected flow. The harbor/return flow remains the connected provider-free acceptance anchor.
