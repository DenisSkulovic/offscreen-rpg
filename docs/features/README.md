...
The existing Storyteller memory umbrella is the shared product/authority owner. Do not implement a parallel memory store or second exploration runner from its broad phase wording. The exact implementation action is exploration X2, not an embedding database or GraphRAG deployment.

The earlier browser/gold-session route remains useful acceptance evidence after the first connected memory return exists; it is no longer the next coding action. The bounded Phase 1 slice of [token-efficient diagnostics](2026-09-20--16-19--token-efficient-diagnostics/PLAN.md), the permanent [committed-time contract](../technical/committed-time.md), and the essential [bounded-cost B2](2026-09-19--19-08--bounded-storyteller-cost/PLAN.md) retry boundary are implemented.
...
Memory exploration X2 is the nearest prepared work. The item-state checkpoint at `a38076c` admits dynamic items; synchronizing its canonical inventory projection remains a contained storage seam and must not displace the memory program unless it blocks the connected flow. The harbor/return flow remains the connected acceptance anchor.

| Feature | Phases | Ownership |
| --- | --- | --- |
| [Token-efficient diagnostics](2026-09-20--16-19--token-efficient-diagnostics/FEATURE.md) | 1 packet map/drill-down → 2 playthrough failure digest → 3 cross-run comparison | Layered manifests and bounded evidence retrieval for economical agent debugging |
| [Story forks and alternate continuities](2026-09-20--18-28--story-forks/FEATURE.md) | 1 current committed checkpoint → 2 historical state snapshots → 3 branch navigation/tree | First-class independent timelines for testing and future player “what if?” play |
