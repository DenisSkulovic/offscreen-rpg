...
- The fresh consequence proposed a meaningless zero-delta quantity change beside a real departure effect. Quantity-change schemas now require a non-zero delta, and the contract states that omission means no quantity change.
...
5. Prompt v8 defines the neutral promotion threshold, and the maintained provider-free return benchmark proves a source-linked identity and relationship can cross unrelated material and appear as current records in a later task. Continue the fresh story by asking about the tally, inspect any document change, then leave and test later recall before a 10–15-turn attempt;
