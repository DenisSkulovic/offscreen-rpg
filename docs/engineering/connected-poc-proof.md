...
The proof is **not ready or passed**. Two historical Sol stories and their weaker Luna contrasts established that Sol materially improves prose, choice quality, continuity and process selection. Fresh v16 story `0da94438-d05b-4c86-821c-0dc61405bb4c` supplies the strongest current authority evidence: six successful Storyteller generations, an immutable 1,800-fictional-second warehouse process, one six-septim payment, one occurrence closure and no settled-effect replay in later plans. It also supplies the decisive continuity failure. Three meaningful exchanges with the same clerk created no durable identity, relationship, thread or note, and leaving the warehouse restarted the scene with no recall documents. The maintained manifest still contained only the start identities and release thread. This is a successful v16 checkpoint, not a connected POC pass.
...
- The fresh v16 clerk sequence makes the same boundary conclusive: active-scene prose can sustain a short exchange, but scene restart discards it when no document is materialized or recalled. Mechanical action plans also lack the narrative-choice dependency sidecar needed to request exact canonical context for their consequences. This structural handoff now precedes more paid play.
...
The next evidence tranche is provider-free S3 continuity work followed by the connected diagnostic:
...
4. add bounded canonical dependencies to mechanical action plans, resolve task-local handles/change indices to stable document IDs at publication and load the exact current documents for the selected consequence;
5. use the maintained Socucius identity in a provider-free interaction → unrelated play → leave → return proof. Keep empty dependencies valid for worlds or scenes without social entities, and do not infer durable biographies from prose nouns;
6. resume the 10–15-turn Sol acceptance campaign only after this handoff works, then evaluate narrow profile tuning. Preserve the current Sol tendency to end scenes with explanatory relational summaries as a hypothesis; do not hard-code a generic prose preference from one story.
