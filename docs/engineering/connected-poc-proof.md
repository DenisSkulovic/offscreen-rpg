...
- After selecting the tally inquiry, Sol produced grounded prose and useful choices but repeated that zero-delta pattern in one sibling road plan. The exact settled response was rejected with `invalid_output`, leaving the 60-second action committed and the story held. Prompt `storyteller.v10` now names the rule explicitly. Because ordinary one-shot turns have no explicit repair path, this otherwise-useful billed response cannot be published or safely retried; recovery is now the earliest blocker.
