...
5. Prompt v8 defines the neutral promotion threshold, and the maintained provider-free return benchmark now proves a source-linked identity and relationship can cross unrelated material and appear as current records in a later task. Prove the same behavior in the next live diagnostic before a 10–15-turn attempt;
