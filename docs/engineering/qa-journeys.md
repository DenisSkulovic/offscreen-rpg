...
10. Interrupt each model round after its exact request is persisted, after its raw response is captured and after its memory action is persisted. Resume with the same generation and require the same attempt ID/request hash, no repeated captured response, at-most-repeatable read-only canonical lookup, and one final candidate. Provider variants must additionally prove that review, reservation and settlement all name that same attempt before live multi-round execution is enabled.
