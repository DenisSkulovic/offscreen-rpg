...
The stage-separated evaluator must preserve failure attribution. An empty/broken retriever fails retrieval; a complete retrieval whose evidence is assembled but ignored by scripted generation passes retrieval/assembly and fails generation use. The initial linear baseline is intentionally retained even when poor so later indexed, semantic or agentic routes cannot improve their headline by changing the oracle or hiding forbidden/noisy context.
...
- long-story memory inside the reusable start-package case: version 17 preserves the contrasting-world oracle, retrieval postures and the provider-free Greywake saved request → crash recovery → durable snapshot → exact source-read → replayed final-candidate path, plus replay-stable classified failure. These stages prove retrieval/controller plumbing and authority boundaries, not model comprehension or a published turn.
...
4. Persist and restore the exploration snapshot, then inspect the discovered current favor and read its authorized original source. Confirm exact manifest identity is rechecked, four reads remain within the recipe ceiling and private/obsolete material is excluded.
5. Fail immediately after a context request is durably accepted but before its reads complete. Resume and confirm the hashed saved request executes before any new Storyteller decision; its reads count once in the accepted snapshot.
...
8. Require a final Storyteller result that uses recovered evidence without inventing unsupported state and passes through ordinary publication. The connected deterministic Greywake fixture proves that wiring with the revised cliff-stairs route. At the same root, require balanced retrieval to add optional leads beyond minimal without changing the path/authority/visibility of shared evidence or admitting developer-private material. Run the abstract membrane task through the same executor and reject person, tavern, wage, calendar and quest defaults. Because both providers are authored to use their packets, these are not evidence for model comprehension or narrative quality.
...
10. Interrupt each model round after its exact request is persisted, after its normalized output/settlement delivery is captured, during settlement, and after its memory action is persisted. Resume with the same generation and require the same attempt ID/request hash, repeated idempotent settlement from the saved delivery, no repeated model source, at-most-repeatable read-only canonical lookup, and one final candidate. The focused disposable-database fake-provider variant covers held review with exact release or rejection, confirmed-unsent authority denial, successful settlement/completion, known invalid output with retained settled delivery, ambiguous delivery with retained liability, no-call replay, intermediate settlement followed by terminal stale-root closure with its known charge unchanged, and one malformed-final repair sharing the original round/operation allowance. A path that already consumed all discovery rounds must fail rather than receive an extra repair call.
11. Run the 2,000-scene profile and the connected playable-return case. Record query work, candidates, reads, bytes, rounds, latency and provider spend; keep retrieval, assembly and generation-use failures separate.
12. Admit episode or thread cards only when they improve a labelled return case. Confirm stable source ranges, visible missing/stale coverage, same-name and branch isolation, and that a derived card cannot override current mechanical state.
13. Compare any local semantic route with indexed lexical under the same final packet budget. Require a meaningful paraphrase-recall win without unacceptable forbidden/noise regression, visible model/index/version coverage, and honest lexical fallback before adoption.
14. Compare explicit-link traversal and query-focused broad synthesis with flat retrieval on labelled multi-hop/global questions. Add graph/community or hierarchy machinery only if that extra layer wins under its separately declared budget.
...
12. Ask a possibilities question with creative work disabled or over its query limit, then inject too many returned candidates, cumulative leads or final directions. Confirm the canonical dispatcher records the internal creative route, caps before allocating excess handles, reports lead exhaustion explicitly, and the controller rejects an over-limit custom snapshot as durable `creative-limit`; ordinary evidence questions remain governed by their parent limits.
13. Reserve and settle at least two exact provider packets beneath one captured operation envelope. Confirm each attempt and usage window records only its dispatch slice, intermediate settlement leaves the operation open, cumulative rounds/tokens/money never exceed or renew the parent ceiling, and only the final settlement closes the operation. Retry the same attempt identity with a different packet or slice and require rejection before transport.
...
Run `pnpm chamber:memory-packet` before authorizing memory inference. Verify the evidence identifies the maintained Greywake corpus, memory state `held`, exactly one awaiting review, zero provider attempts, a first-round memory-exploration request and the bounded `ask_memory`/`read_memory` contract. The selected intention should ask Mira for the exact old brass-key hiding place: the answer is in passage history but is not stated by the compact canonical records already supplied. Inspect section sizes and allowances before raw prose. This proves only that the real controller reaches the reviewed zero-spend boundary; relevance, final-answer use and narrative quality require later deterministic or explicitly authorized evaluation.

For `pnpm chamber:memory-evaluation-packet -- --config=...`, additionally verify the held packet names the configured model and route, the manifest binds the same generation, attempt ID and SHA-256, and the captured recipe reports exactly two model rounds, one read, one in-flight call and zero optional inference categories. Confirm the route validity interval is current and both captured prices plus the reservation are zero. This remains credential-free packet evidence; it is not permission to release either round.

For a separately deliberate `chamber:memory-evaluation-run`, inspect the compact report before raw responses. There must be one operation, no more than two distinct attempts, zero reported and calculated microusd for every dispatched attempt, matched reconciliation, and no retry. If a second review exists, its attempt ID and hash must differ from round one and its release must follow round-one settlement plus route reverification. Confirm the report distinguishes a model that answered immediately from one that requested memory, and records retrieved-read/round counts without copying generated prose. Treat uncertainty or missing settlement as a stopped run, never as zero spend.

Before using SSE for such a run, inspect that the held packet itself contains `stream: true` and final usage reporting; transport mode must change the packet hash. With injected transport, split a valid structured result across events and require the ordinary result validator and usage settlement to succeed. Then omit the terminal event or usage and require an uncertain outcome that retains a valid response-header generation ID. These checks remain provider-free and do not prove that a selected live route emits headers before its timeout.
