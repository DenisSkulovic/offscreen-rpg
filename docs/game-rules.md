...
Long-running work, travel and study contribute rule-defined progress at meaningful tick boundaries. The implemented contribution contract captures a content-defined target, contribution per productive boundary and cadence; exact elapsed clock progress is stored separately. A process can pause while retaining committed progress and does not complete merely because ticks elapsed. Check-only boundaries can advance the clock or interrupt without contributing. Waiting, traversal and other process families remain designs rather than implemented semantics. Encounter occurrence and action success remain distinct checks.
...
On the opening preview, select either mechanical rehearsal before Start. Pineapple and microbe use the same immediate-action contract: review exposes public labels while the generated result retains private intentions, prerequisites, checks, DCs and outcome branches. Start copies those exact reviewed plans into the story, and the browser later submits only the offer identity and selected path. The offline source is deterministic fixture behavior; it proves the boundary, not general model quality.

Immediate automatic actions and ability checks commit through the same receipt path. Outcomes may explicitly declare a bounded durable story fact when the current action establishes something not previously represented; updating an existing character fact remains a different effect. Immediate plans may require a minimum declared quantity, which is checked before any roll. Latest 100 activity roll records and recent immediate receipts are exposed in the play view. The contribution process policy/runtime is implemented but not yet admitted from a playable offer; interruption resumption, full combat, study/level progression and live-model evaluation remain outside this subset. Existing narrative stories are not silently assigned character sheets. Rules attribution is available at `/rules` and in [the attribution document](rules-attribution.md).
