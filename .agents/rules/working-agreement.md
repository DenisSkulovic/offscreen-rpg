...
On this Windows workstation, read [Local dependencies](../../docs/development.md#local-dependencies) before starting or repairing project infrastructure. The verified path is Docker Engine and Compose inside the `Ubuntu-24.04` WSL distribution, using WSL mirrored networking and published loopback ports; keep the distribution alive while its services are needed. Docker Desktop is not a project dependency, and its known WSL socket-bridge failure must not trigger another reinstall by default. Diagnose WSL connectivity first and deliberately reevaluate this workaround only when new evidence warrants it.
